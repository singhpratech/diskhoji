...
pub fn scan_root(path: &Path, prog: &Progress) -> Option<LocalNode> {
    ...
    let mut root = scan_dir(path, dev, prog, &seen, 0)?;
    root.name = path.to_string_lossy().into_owned();
    Some(root)
}

fn scan_dir(path: &Path, dev: u64, prog: &Progress, seen: &SeenLinks, depth: u32) -> Option<LocalNode> {
    ...
    for ent in rd {
        ...
        if ft.is_dir() {
            ...
            let size = match ent.metadata() {
                Ok(md) => file_size_on_disk(&ent, &md, seen),
                Err(_) => 0,
            };
    ...
    let scanned: Vec<LocalNode> = subdirs
        .into_par_iter()
        .filter_map(|p| scan_dir(&p, dev, prog, seen, depth + 1))
        .collect();

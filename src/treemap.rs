...
pub const SLOT_FREE: u8 = 252; // free space on the volume (d = 3)
pub const SLOT_UNKNOWN: u8 = 251; // used but not measurable: MFT, unreadable folders (d = 4)
...
pub struct Rect {
    ...
    pub d: u8, // 0 file, 1 dir-aggregate, 2 merged-small-items, 3 free space, 4 unknown
    pub n: String,
    pub z: u64, // bytes
    pub t: u32, // top-level ancestor under the zoom root (for colour-by-folder)
    pub m: i64, // modified, unix seconds (for colour-by-age)
}
...
/// `extras` are WinDirStat-style pseudo blocks drawn beside the tree at this
/// zoom level — (label, bytes, slot, kind) — e.g. free space and unknown.
pub fn layout(
    store: &Store,
    root: u32,
    w: f64,
    h: f64,
    extras: &[(String, u64, u8, u8)],
) -> (Vec<Rect>, Vec<DirRect>) {
    let mut leafs = Vec::new();
    let mut dirs = Vec::new();
    let rsize = store.nodes[root as usize].size;
    let extras: Vec<&(String, u64, u8, u8)> = extras.iter().filter(|e| e.1 > 0).collect();
    if rsize == 0 && extras.is_empty() {
        return (leafs, dirs);
    }
    let mut heap: std::collections::BinaryHeap<Work> = std::collections::BinaryHeap::new();
    if extras.is_empty() {
        heap.push(Work { area: w * h, id: root, x: 0.0, y: 0.0, w, h, depth: 0, top: root });
    } else {
        // the tree and the pseudo blocks share the canvas by size
        let mut items: Vec<(Option<&(String, u64, u8, u8)>, f64)> = vec![(None, rsize as f64)];
        for e in &extras {
            items.push((Some(e), e.1 as f64));
        }
        items.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
        let sizes: Vec<f64> = items.iter().map(|i| i.1).collect();
        let placed = squarify(&sizes, 0.0, 0.0, w, h);
        for ((ex, _), r) in items.into_iter().zip(placed) {
            match ex {
                None => {
                    if rsize > 0 {
                        heap.push(Work {
                ...
                Some((label, bytes, slot, kind)) => leafs.push(Rect {
                    i: root,
                    x: r1(r.0),
                    y: r1(r.1),
                    w: r1(r.2),
...
fn expand(
    ...
    // children of the zoom root are their own "top" folder; deeper nodes inherit
    let top_of = |cid: u32| if depth == 0 { cid } else { wk.top };

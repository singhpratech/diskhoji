...
- **A game on board.** *The Seeker's Voyage* — a Chrome-dino-style side-scroller — ships inside the app: click **Play game** (in the toolbar, or on the landing screen while you decide what to scan). Leap the corrupt reefs, gather amber bytes; Space/↑ to jump, release early for a short hop, ↓ to dive. Fully offline, best score saved locally. The same game is playable in `--web` mode and on [diskhoji.org](https://diskhoji.org/#voyage).

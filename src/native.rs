//! Standalone native window (egui) — the default way Diskhoji runs.

use crate::scan::{self, ScanOptions, Store};
use crate::treemap::{self, DirRect, Rect as TRect, SLOT_DIRAGG, SLOT_FREE, SLOT_SMALL, SLOT_UNKNOWN};
use crate::{
    list_roots, open_terminal, open_with_default, remove_path, reveal_in_file_manager,
    sorted_children, start_scan, store_disk_usage, treemap_extras, App as Core, RootEntry,
    ScanTarget, SortKey,
};
use std::collections::BTreeSet;
use eframe::egui::{
    self, Align2, Color32, FontFamily, FontId, Pos2, Rect, RichText, Rounding, Sense, Stroke,
    TextStyle, Vec2,
};
use std::path::PathBuf;
use std::sync::atomic::Ordering;
use std::sync::Arc;
use std::time::Instant;

const UNITS: [&str; 6] = ["B", "KiB", "MiB", "GiB", "TiB", "PiB"];

fn fmt_bytes(n: u64) -> String {
    let mut v = n as f64;
    let mut u = 0;
    while v >= 1024.0 && u < UNITS.len() - 1 {
        v /= 1024.0;
        u += 1;
    }
    if u == 0 || v >= 100.0 {
        format!("{:.0} {}", v, UNITS[u])
    } else if v >= 10.0 {
        format!("{:.1} {}", v, UNITS[u])
    } else {
        format!("{:.2} {}", v, UNITS[u])
    }
}

fn fmt_n(n: u64) -> String {
    let s = n.to_string();
    let mut out = String::with_capacity(s.len() + s.len() / 3);
    for (i, c) in s.chars().enumerate() {
        if i > 0 && (s.len() - i) % 3 == 0 {
            out.push(',');
        }
        out.push(c);
    }
    out
}

fn fmt_pct(p: f64) -> String {
    if p >= 10.0 {
        format!("{:.0}%", p)
    } else {
        format!("{:.1}%", p)
    }
}

fn fmt_time(ms: u64) -> String {
    if ms >= 1000 {
        format!("{:.1} s", ms as f64 / 1000.0)
    } else {
        format!("{} ms", ms)
    }
}

fn hex(c: &str) -> Color32 {
    let b = u32::from_str_radix(&c[1..], 16).unwrap_or(0);
    Color32::from_rgb((b >> 16) as u8, (b >> 8) as u8, b as u8)
}

// ---------- theme ----------

#[derive(Clone, Copy, PartialEq)]
pub enum ThemeKind {
    Dark,
    Light,
}

struct Theme {
    bg: Color32,
    well: Color32,
    panel: Color32,
    panel2: Color32,
    line: Color32,
    ink: Color32,
    ink2: Color32,
    ink3: Color32,
    acc: Color32,
    danger: Color32,
    slots: [Color32; 8],
    other: Color32,
    diragg: Color32,
    small: Color32,
    free: Color32,
    unknown: Color32,
    heat: [Color32; 5],
}

fn theme(kind: ThemeKind) -> Theme {
    match kind {
        ThemeKind::Dark => Theme {
            bg: hex("#0E1116"),
            well: hex("#0A0D12"),
            panel: hex("#141922"),
            panel2: hex("#1B2130"),
            line: hex("#232C3A"),
            ink: hex("#E8ECF4"),
            ink2: hex("#A6B0C3"),
            ink3: hex("#7A879E"),
            acc: hex("#E5B96B"),
            danger: hex("#E66767"),
            slots: [
                hex("#3987e5"),
                hex("#199e70"),
                hex("#c98500"),
                hex("#008300"),
                hex("#9085e9"),
                hex("#e66767"),
                hex("#d55181"),
                hex("#d95926"),
            ],
            other: hex("#566072"),
            diragg: hex("#3E4654"),
            small: hex("#2E3542"),
            free: hex("#1F2A3A"),
            unknown: hex("#4A3A5A"),
            heat: [
                hex("#1D2531"),
                hex("#0E4429"),
                hex("#006D32"),
                hex("#26A641"),
                hex("#39D353"),
            ],
        },
        ThemeKind::Light => Theme {
            bg: hex("#F2EEE6"),
            well: hex("#E9E4D8"),
            panel: hex("#FAF7F0"),
            panel2: hex("#F1ECE0"),
            line: hex("#D9D2C3"),
            ink: hex("#1D2532"),
            ink2: hex("#4A566B"),
            ink3: hex("#75808F"),
            acc: hex("#9C7326"),
            danger: hex("#B03A3A"),
            slots: [
                hex("#2F6FBF"),
                hex("#0F7A52"),
                hex("#9A6A00"),
                hex("#1F6B00"),
                hex("#6F63C9"),
                hex("#C24848"),
                hex("#AD3C64"),
                hex("#B0451C"),
            ],
            other: hex("#8A94A6"),
            diragg: hex("#B9C0CC"),
            small: hex("#C9CFD8"),
            free: hex("#DDE6F0"),
            unknown: hex("#D9CCE3"),
            heat: [
                hex("#E4DFD2"),
                hex("#9BE9A8"),
                hex("#40C463"),
                hex("#30A14E"),
                hex("#216E39"),
            ],
        },
    }
}

impl Theme {
    fn slot_color(&self, s: u8) -> Color32 {
        match s {
            0..=7 => self.slots[s as usize],
            SLOT_SMALL => self.small,
            SLOT_DIRAGG => self.diragg,
            SLOT_FREE => self.free,
            SLOT_UNKNOWN => self.unknown,
            _ => self.other,
        }
    }
}

fn lighten(c: Color32, f: f32) -> Color32 {
    let l = |v: u8| (v as f32 + (255.0 - v as f32) * f) as u8;
    Color32::from_rgb(l(c.r()), l(c.g()), l(c.b()))
}

fn darken(c: Color32, f: f32) -> Color32 {
    let d = |v: u8| (v as f32 * (1.0 - f)) as u8;
    Color32::from_rgb(d(c.r()), d(c.g()), d(c.b()))
}

// ---------- state ----------

#[derive(Clone, Copy, PartialEq)]
enum View {
    Map,
    Heat,
    Rings,
}

impl View {
    fn code(self) -> u8 {
        match self {
            View::Map => 0,
            View::Heat => 1,
            View::Rings => 2,
        }
    }
}

struct Sector {
    id: u32,
    r0: f32,
    r1: f32,
    a0: f32,
    a1: f32,
    slot: u8,
    d: u8, // 0 file, 1 dir, 2 merged-small
    name: String,
    size: u64,
}

struct ExtRow {
    name: String,
    desc: &'static str,
    bytes: u64,
    files: u64,
    slot: u8,
}

#[derive(Clone, Copy, PartialEq, serde::Serialize, serde::Deserialize)]
enum ColorMode {
    Type,
    Folder,
    Age,
}

/// UI preferences that survive restarts (diskhoji-ui.json).
#[derive(Clone, serde::Serialize, serde::Deserialize)]
struct UiPrefs {
    show_free: bool,
    show_unknown: bool,
    color_mode: ColorMode,
    cushions: bool,
    outlines: bool,
    labels: bool,
}

impl Default for UiPrefs {
    fn default() -> Self {
        UiPrefs {
            show_free: false,
            show_unknown: false,
            color_mode: ColorMode::Type,
            cushions: true,
            outlines: true,
            labels: true,
        }
    }
}

#[derive(Clone)]
struct Props {
    name: String,
    path: String,
    dir: bool,
    size: u64,
    files: u32,
    subdirs: u32,
    mtime: i64,
    pct_parent: f64,
    pct_total: f64,
    ext: String,
}

#[derive(Clone, Copy, PartialEq)]
enum TypesSort {
    Bytes,
    Files,
    Name,
}

/// Age buckets for colour-by-age, oldest last.
const AGE_BUCKETS: [(&str, i64); 6] = [
    ("this week", 7 * 86_400),
    ("this month", 31 * 86_400),
    ("6 months", 183 * 86_400),
    ("this year", 366 * 86_400),
    ("3 years", 3 * 366 * 86_400),
    ("older", i64::MAX),
];

fn age_bucket(now: i64, mtime: i64) -> usize {
    let age = now - mtime;
    AGE_BUCKETS.iter().position(|b| age < b.1).unwrap_or(AGE_BUCKETS.len() - 1)
}

fn now_secs() -> i64 {
    std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs() as i64)
        .unwrap_or(0)
}

struct BigRow {
    id: u32,
    name: String,
    size: u64,
    slot: u8,
}

struct Snap {
    root: String,
    bytes: u64,
    files: u64,
    dirs: u64,
    errors: u64,
    skipped: u64,
    elapsed: u64,
    disk_total: u64,
    disk_free: u64,
    multi: bool,
    volumes: usize,
    exts: Vec<ExtRow>,
    largest: Vec<BigRow>,
}

struct HeatCell {
    id: u32,
    rect: Rect,
    lvl: u8,
    slot: u8,
    dir: bool,
    name: String,
    size: u64,
}

enum Act {
    Zoom(u32),
    ZoomParent(u32),
    Select(u32),
    /// ctrl-click: add/remove from the multi-selection
    ToggleSelect(u32),
    AskDelete(u32),
    /// delete everything currently selected
    AskDeleteSelected,
    Reveal(u32),
    Open(u32),
    Terminal(u32),
    CopyPath(u32),
    Props(u32),
    RescanFolder(u32),
    ExportCsv(u32),
    ToggleExt(u8),
    /// expand ancestors + scroll the explorer to a node
    Locate(u32),
}

#[derive(Clone)]
struct DelTarget {
    id: u32,
    gen: u64,
    name: String,
    path: String,
    size: u64,
    dir: bool,
    files: u32,
}

pub struct Native {
    core: Arc<Core>,
    theme_kind: ThemeKind,
    auto_update: bool, // opt-in: check GitHub for a newer version on launch (default off)
    view: View,
    snap: Option<Snap>,
    gen_seen: u64,
    zoom: u32,
    crumbs: Vec<(u32, String)>,
    rects: Vec<TRect>,
    dir_rects: Vec<DirRect>,
    heat_cells: Vec<HeatCell>,
    heat_more: u32,
    sectors: Vec<Sector>,
    ring_label: (String, String),
    cached_mesh: egui::Mesh,
    cached_labels: Vec<(Pos2, String, bool)>,
    picked: Arc<std::sync::Mutex<Option<Option<String>>>>,
    picking: bool,
    ctx_target: Option<(u32, String, u64, bool, u8)>,
    map_key: (u64, u32, u32, u32, u8),
    sel: Option<u32>,
    /// every selected node (ctrl-click); `sel` is the most recent one
    sel_multi: BTreeSet<u32>,
    ext_sel: Option<u8>,
    delete_target: Option<Vec<DelTarget>>,
    /// worker results per target: (path result) — polled each frame
    pending_delete: Option<(Arc<std::sync::Mutex<Option<Vec<(DelTarget, std::io::Result<()>)>>>>, usize, bool)>,
    deleted_since_scan: bool,
    sort_key: SortKey,
    sort_desc: bool,
    search: String,
    search_hits: Vec<u32>,
    search_gen: u64,
    search_zoom: u32,
    focus_search: bool,
    prefs: UiPrefs,
    prefs_json: PathBuf,
    types_sort: (TypesSort, bool),
    props: Option<Props>,
    trash_confirm: Option<u64>,
    trash_busy: Option<Arc<std::sync::Mutex<Option<Result<u64, String>>>>>,
    show_opts: bool,
    excludes_text: String,
    skip_hidden: bool,
    /// restore zoom to this path after a subtree rescan lands
    restore_zoom: Option<String>,
    visible_rows: Vec<u32>,
    scroll_to: Option<u32>,
    expand_req: Vec<u32>,
    legend_now: i64,
    toast: Option<(String, Instant, bool)>,
    pending_copy: Option<String>,
    path_input: String,
    roots: Vec<RootEntry>,
    scan_started: Option<Instant>,
    prefs_path: PathBuf,
    logo: Option<egui::TextureHandle>,
    show_landing: bool,
    show_about: bool,
    show_game: bool,
    voyage: crate::voyage::Voyage,
    update: Arc<std::sync::Mutex<UpdateState>>,
    elevated: bool,
    show_elevate: bool,
    elevate_never: bool,
    // unix: the pkexec/osascript child while its auth dialog is up (always
    // None on Windows — UAC consent resolves synchronously instead)
    elevate_child: Option<std::process::Child>,
    elevate_err: Option<String>,
}

#[cfg(target_os = "linux")]
const ELEVATE_HOW: &str =
    "Uses your system's polkit prompt (pkexec) — the same dialog other admin tools use.";
#[cfg(target_os = "macos")]
const ELEVATE_HOW: &str = "Uses the standard macOS administrator dialog.";
#[cfg(windows)]
const ELEVATE_HOW: &str = "Uses the standard Windows UAC prompt.";

#[cfg(target_os = "linux")]
pub(crate) fn pick_folder_blocking() -> Option<String> {
    for (cmd, args) in [
        ("zenity", &["--file-selection", "--directory"][..]),
        ("kdialog", &["--getexistingdirectory", "."][..]),
    ] {
        match std::process::Command::new(cmd).args(args).output() {
            Ok(out) if out.status.success() => {
                let s = String::from_utf8_lossy(&out.stdout).trim().to_string();
                if !s.is_empty() {
                    return Some(s);
                }
            }
            Ok(_) => return None, // dialog shown, user cancelled
            Err(_) => continue,   // tool not installed, try the next
        }
    }
    None
}

#[cfg(target_os = "macos")]
pub(crate) fn pick_folder_blocking() -> Option<String> {
    let out = std::process::Command::new("osascript")
        .args(["-e", "POSIX path of (choose folder)"])
        .output()
        .ok()?;
    if !out.status.success() {
        return None;
    }
    let s = String::from_utf8_lossy(&out.stdout).trim().to_string();
    (!s.is_empty()).then_some(s)
}

#[cfg(windows)]
pub(crate) fn pick_folder_blocking() -> Option<String> {
    let out = std::process::Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "Add-Type -AssemblyName System.Windows.Forms; \
             $d = New-Object System.Windows.Forms.FolderBrowserDialog; \
             if ($d.ShowDialog() -eq 'OK') { $d.SelectedPath }",
        ])
        .output()
        .ok()?;
    let s = String::from_utf8_lossy(&out.stdout).trim().to_string();
    (!s.is_empty()).then_some(s)
}

// ---------- update checker (GitHub releases, no extra deps: shells to curl) ----------

#[derive(Clone)]
enum UpdateState {
    Idle, // default: no check has run — the app has made no network call
    Checking,
    UpToDate,
    Available { ver: String, url: String },
    Failed,
}

fn parse_ver(s: &str) -> Option<(u32, u32, u32)> {
    let s = s.trim().trim_start_matches('v');
    let mut it = s.split(|c: char| c == '.' || c == '-' || c == '+');
    let a = it.next()?.parse().ok()?;
    let b = it.next()?.parse().ok()?;
    let c = it.next().unwrap_or("0").parse().unwrap_or(0);
    Some((a, b, c))
}

fn fetch_latest_release() -> Option<(String, String)> {
    let out = std::process::Command::new("curl")
        .args([
            "-fsSL",
            "--max-time",
            "8",
            "-H",
            "Accept: application/vnd.github+json",
            "https://api.github.com/repos/singhpratech/diskhoji/releases/latest",
        ])
        .output()
        .ok()?;
    if !out.status.success() {
        return None;
    }
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).ok()?;
    let tag = v.get("tag_name")?.as_str()?.to_string();
    let url = v
        .get("html_url")
        .and_then(|u| u.as_str())
        .unwrap_or("https://github.com/singhpratech/diskhoji/releases")
        .to_string();
    Some((tag, url))
}

fn spawn_update_check(slot: Arc<std::sync::Mutex<UpdateState>>) {
    if let Ok(mut g) = slot.lock() {
        *g = UpdateState::Checking;
    }
    std::thread::spawn(move || {
        let cur = parse_ver(env!("CARGO_PKG_VERSION")).unwrap_or((0, 0, 0));
        let st = match fetch_latest_release() {
            Some((tag, url)) => match parse_ver(&tag) {
                Some(latest) if latest > cur => UpdateState::Available { ver: tag, url },
                Some(_) => UpdateState::UpToDate,
                None => UpdateState::Failed,
            },
            None => UpdateState::Failed,
        };
        if let Ok(mut g) = slot.lock() {
            *g = st;
        }
    });
}

pub(crate) fn dirs_config() -> PathBuf {
    if let Ok(x) = std::env::var("XDG_CONFIG_HOME") {
        return PathBuf::from(x);
    }
    #[cfg(windows)]
    if let Ok(a) = std::env::var("APPDATA") {
        return PathBuf::from(a);
    }
    PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| ".".into())).join(".config")
}

fn apply_style(ctx: &egui::Context, kind: ThemeKind) {
    let t = theme(kind);
    let mut style = (*ctx.style()).clone();
    style.text_styles = [
        (TextStyle::Heading, FontId::new(26.0, FontFamily::Monospace)),
        (TextStyle::Body, FontId::new(16.5, FontFamily::Monospace)),
        (TextStyle::Monospace, FontId::new(16.5, FontFamily::Monospace)),
        (TextStyle::Button, FontId::new(16.0, FontFamily::Monospace)),
        (TextStyle::Small, FontId::new(13.5, FontFamily::Monospace)),
    ]
    .into();
    style.spacing.item_spacing = Vec2::new(10.0, 8.0);
    style.spacing.button_padding = Vec2::new(14.0, 7.0);
    style.spacing.scroll.bar_width = 10.0;

    let mut v = match kind {
        ThemeKind::Dark => egui::Visuals::dark(),
        ThemeKind::Light => egui::Visuals::light(),
    };
    v.panel_fill = t.panel;
    v.window_fill = t.panel2;
    v.extreme_bg_color = t.well;
    v.faint_bg_color = t.panel2;
    v.override_text_color = Some(t.ink);
    v.selection.bg_fill = t.acc.gamma_multiply(0.35);
    v.selection.stroke = Stroke::new(1.0, t.acc);
    v.hyperlink_color = t.acc;
    v.widgets.noninteractive.bg_stroke = Stroke::new(1.0, t.line);
    v.widgets.noninteractive.fg_stroke = Stroke::new(1.0, t.ink2);
    v.widgets.inactive.bg_fill = t.panel2;
    v.widgets.inactive.weak_bg_fill = t.panel2;
    v.widgets.inactive.bg_stroke = Stroke::new(1.0, t.line);
    v.widgets.inactive.fg_stroke = Stroke::new(1.0, t.ink2);
    v.widgets.hovered.bg_fill = lighten(t.panel2, 0.06);
    v.widgets.hovered.weak_bg_fill = lighten(t.panel2, 0.06);
    v.widgets.hovered.bg_stroke = Stroke::new(1.0, t.acc.gamma_multiply(0.7));
    v.widgets.hovered.fg_stroke = Stroke::new(1.2, t.ink);
    v.widgets.active.bg_fill = t.acc.gamma_multiply(0.3);
    v.widgets.active.fg_stroke = Stroke::new(1.2, t.ink);
    v.window_stroke = Stroke::new(1.0, t.line);
    style.visuals = v;
    ctx.set_style(style);
}

impl Native {
    fn new(cc: &eframe::CreationContext<'_>, core: Arc<Core>, takeover: Option<u32>) -> Self {
        // The pre-elevation instance is retired only HERE — our window and GL
        // context already exist — so an elevated launch that dies at startup
        // can never take the user's working window down with it.
        if let Some(pid) = takeover {
            crate::elevate::takeover(pid);
            #[cfg(target_os = "linux")]
            crate::elevate::revoke_root_x_access();
        }
        let prefs_path = dirs_config().join("diskhoji-prefs");
        let theme_kind = match std::fs::read_to_string(&prefs_path) {
            Ok(s) if s.trim() == "dark" => ThemeKind::Dark,
            _ => ThemeKind::Light, // light is the default; only an explicit saved "dark" overrides
        };
        // Update checks are OPT-IN. With no opt-in file, the app never touches the network.
        let auto_update = std::fs::read_to_string(dirs_config().join("diskhoji-updatecheck"))
            .map(|s| s.trim() == "on")
            .unwrap_or(false);
        let elevated = crate::elevate::is_elevated();
        // WinDirStat-style: an unelevated launch offers to restart with
        // admin rights every time, unless the user opted out for good.
        let elevate_never = std::fs::read_to_string(dirs_config().join("diskhoji-elevate"))
            .map(|s| s.trim() == "never")
            .unwrap_or(false);
        let roots = list_roots();
        let path_input = roots
            .first()
            .map(|r| r.path.clone())
            .unwrap_or_else(|| "/".into());
        let prefs_json = dirs_config().join("diskhoji-ui.json");
        let prefs: UiPrefs = std::fs::read_to_string(&prefs_json)
            .ok()
            .and_then(|s| serde_json::from_str(&s).ok())
            .unwrap_or_default();
        let (skip_hidden, excludes_text) = {
            let o = core.opts.read().unwrap();
            (o.skip_hidden, o.excludes.join(", "))
        };
        apply_style(&cc.egui_ctx, theme_kind);
        // Install DejaVuSans as a fallback so glyphs missing from the default
        // fonts (common on Windows) render instead of tofu boxes. It is appended
        // to each family's fallback list, so it only fills gaps and the app's
        // primary typeface — and its look — is preserved.
        let mut fonts = egui::FontDefinitions::default();
        fonts.font_data.insert(
            "DejaVuSans".to_owned(),
            egui::FontData::from_static(include_bytes!("../assets/DejaVuSans.ttf")),
        );
        fonts
            .families
            .entry(egui::FontFamily::Proportional)
            .or_default()
            .push("DejaVuSans".to_owned());
        fonts
            .families
            .entry(egui::FontFamily::Monospace)
            .or_default()
            .push("DejaVuSans".to_owned());
        cc.egui_ctx.set_fonts(fonts);
        let logo = image::load_from_memory(include_bytes!("../assets/icon-64.png"))
            .ok()
            .map(|img| {
                let rgba = img.to_rgba8();
                let (w, h) = rgba.dimensions();
                let ci = egui::ColorImage::from_rgba_unmultiplied(
                    [w as usize, h as usize],
                    rgba.as_raw(),
                );
                cc.egui_ctx
                    .load_texture("logo", ci, egui::TextureOptions::LINEAR)
            });
        Self {
            core,
            theme_kind,
            view: View::Map,
            snap: None,
            gen_seen: 0,
            zoom: 0,
            crumbs: Vec::new(),
            rects: Vec::new(),
            dir_rects: Vec::new(),
            heat_cells: Vec::new(),
            heat_more: 0,
            sectors: Vec::new(),
            ring_label: (String::new(), String::new()),
            cached_mesh: egui::Mesh::default(),
            cached_labels: Vec::new(),
            picked: Arc::new(std::sync::Mutex::new(None)),
            picking: false,
            ctx_target: None,
            map_key: (u64::MAX, 0, 0, 0, 0),
            sel: None,
            sel_multi: BTreeSet::new(),
            ext_sel: None,
            delete_target: None,
            pending_delete: None,
            deleted_since_scan: false,
            sort_key: SortKey::Size,
            sort_desc: true,
            search: String::new(),
            search_hits: Vec::new(),
            search_gen: 0,
            search_zoom: 0,
            focus_search: false,
            prefs,
            prefs_json,
            types_sort: (TypesSort::Bytes, true),
            props: None,
            trash_confirm: None,
            trash_busy: None,
            show_opts: false,
            excludes_text,
            skip_hidden,
            restore_zoom: None,
            visible_rows: Vec::new(),
            scroll_to: None,
            expand_req: Vec::new(),
            legend_now: now_secs(),
            toast: None,
            pending_copy: None,
            path_input,
            roots,
            scan_started: None,
            prefs_path,
            logo,
            show_landing: false,
            show_about: false,
            show_game: false,
            voyage: {
                let best = std::fs::read_to_string(dirs_config().join("diskhoji-voyage-best"))
                    .ok()
                    .and_then(|s| s.trim().parse().ok())
                    .unwrap_or(0);
                crate::voyage::Voyage::new(best)
            },
            auto_update,
            update: {
                let s = Arc::new(std::sync::Mutex::new(UpdateState::Idle));
                if auto_update {
                    spawn_update_check(s.clone()); // only when the user has opted in
                }
                s
            },
            elevated,
            show_elevate: !elevated && !elevate_never,
            elevate_never,
            elevate_child: None,
            elevate_err: None,
        }
    }

    fn about_window(&mut self, ctx: &egui::Context, t: &Theme) {
        if !self.show_about {
            return;
        }
        let screen = ctx.screen_rect();
        egui::Area::new(egui::Id::new("about_dim"))
            .fixed_pos(screen.min)
            .order(egui::Order::Middle)
            .show(ctx, |ui| {
                ui.painter().rect_filled(screen, 0.0, Color32::from_black_alpha(150));
            });

        let mut recheck = false;
        let update = self.update.lock().map(|g| g.clone()).unwrap_or(UpdateState::Failed);
        let green = hex("#199e70");
        egui::Window::new("")
            .title_bar(false)
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .frame(
                egui::Frame::default()
                    .fill(t.panel2)
                    .stroke(Stroke::new(1.0, t.line))
                    .rounding(12.0)
                    .inner_margin(0.0),
            )
            .show(ctx, |ui| {
                ui.set_width(440.0);
                // hero band
                egui::Frame::default()
                    .fill(t.well)
                    .rounding(egui::Rounding { nw: 12.0, ne: 12.0, sw: 0.0, se: 0.0 })
                    .inner_margin(egui::Margin::symmetric(24.0, 22.0))
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            if let Some(tex) = &self.logo {
                                ui.add(
                                    egui::Image::new(tex)
                                        .fit_to_exact_size(Vec2::splat(64.0))
                                        .rounding(14.0),
                                );
                            }
                            ui.add_space(6.0);
                            ui.vertical(|ui| {
                                ui.label(
                                    RichText::new("Diskhoji").size(28.0).strong().color(t.acc),
                                );
                                ui.label(
                                    RichText::new(concat!("version ", env!("CARGO_PKG_VERSION")))
                                        .size(13.5)
                                        .color(t.ink3),
                                );
                                ui.label(
                                    RichText::new("every byte, accounted for.")
                                        .size(13.5)
                                        .color(t.ink2),
                                );
                            });
                        });
                    });

                ui.add_space(2.0);
                egui::Frame::default()
                    .inner_margin(egui::Margin::symmetric(24.0, 18.0))
                    .show(ui, |ui| {
                        ui.label(
                            RichText::new(
                                "A fast, native disk-space analyzer — cushion treemap, heatmap \
                                 grid, and radial rings in one small Rust binary. Linux · macOS · \
                                 Windows.",
                            )
                            .size(13.5)
                            .color(t.ink2),
                        );
                        ui.add_space(6.0);
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Sister app:").size(13.0).color(t.ink3));
                            if ui.link(RichText::new("Kevat — kevat.app").size(13.0).color(t.acc).underline()).clicked() {
                                crate::open_link("https://kevat.app");
                            }
                        });
                        ui.add_space(12.0);

                        // ----- update status band (Notepad++-style) -----
                        egui::Frame::default()
                            .fill(t.well)
                            .stroke(Stroke::new(1.0, t.line))
                            .rounding(8.0)
                            .inner_margin(12.0)
                            .show(ui, |ui| {
                                ui.set_width(ui.available_width());
                                match &update {
                                    UpdateState::Idle => {
                                        ui.label(
                                            RichText::new(
                                                "Diskhoji doesn't check on its own. Ask it to \
                                                 look — only your version number is compared \
                                                 against GitHub, nothing about you is sent.",
                                            )
                                            .size(12.5)
                                            .color(t.ink3),
                                        );
                                        ui.add_space(8.0);
                                        let dark_ink = if self.theme_kind == ThemeKind::Dark {
                                            hex("#161006")
                                        } else {
                                            hex("#FFFFFF")
                                        };
                                        if ui
                                            .add(
                                                egui::Button::new(
                                                    RichText::new("Check for updates")
                                                        .strong()
                                                        .color(dark_ink),
                                                )
                                                .fill(t.acc),
                                            )
                                            .clicked()
                                        {
                                            recheck = true;
                                        }
                                    }
                                    UpdateState::Checking => {
                                        ui.label(
                                            RichText::new("Checking for updates…")
                                                .size(13.5)
                                                .color(t.ink2),
                                        );
                                    }
                                    UpdateState::UpToDate => {
                                        ui.label(
                                            RichText::new("You're on the latest version")
                                                .size(13.5)
                                                .color(green),
                                        );
                                    }
                                    UpdateState::Available { ver, url } => {
                                        ui.horizontal(|ui| {
                                            ui.label(
                                                RichText::new(format!("Update available — {}", ver))
                                                    .size(14.0)
                                                    .strong()
                                                    .color(t.acc),
                                            );
                                        });
                                        ui.label(
                                            RichText::new(
                                                "Close Diskhoji, download the new build, and \
                                                 reinstall (your scans aren't affected).",
                                            )
                                            .size(12.5)
                                            .color(t.ink3),
                                        );
                                        ui.add_space(8.0);
                                        ui.horizontal(|ui| {
                                            let dark_ink = if self.theme_kind == ThemeKind::Dark {
                                                hex("#161006")
                                            } else {
                                                hex("#FFFFFF")
                                            };
                                            if ui
                                                .add(
                                                    egui::Button::new(
                                                        RichText::new("Download update")
                                                            .strong()
                                                            .color(dark_ink),
                                                    )
                                                    .fill(t.acc),
                                                )
                                                .clicked()
                                            {
                                                open_with_default(url);
                                            }
                                            if ui.button("Release notes").clicked() {
                                                open_with_default(url);
                                            }
                                        });
                                    }
                                    UpdateState::Failed => {
                                        ui.horizontal(|ui| {
                                            ui.label(
                                                RichText::new("Couldn't reach GitHub")
                                                    .size(13.5)
                                                    .color(t.ink3),
                                            );
                                            if ui.button("Retry").clicked() {
                                                recheck = true;
                                            }
                                        });
                                    }
                                }
                                if !matches!(update, UpdateState::Checking | UpdateState::Idle) {
                                    ui.add_space(6.0);
                                    if ui
                                        .link(RichText::new("Check again").size(12.5).color(t.ink3))
                                        .clicked()
                                    {
                                        recheck = true;
                                    }
                                }
                                ui.add_space(8.0);
                                ui.separator();
                                ui.add_space(6.0);
                                let mut auto = self.auto_update;
                                if ui
                                    .checkbox(
                                        &mut auto,
                                        RichText::new("Check for updates when Diskhoji starts")
                                            .size(12.5)
                                            .color(t.ink2),
                                    )
                                    .changed()
                                {
                                    self.auto_update = auto;
                                    let _ = std::fs::create_dir_all(dirs_config());
                                    let _ = std::fs::write(
                                        dirs_config().join("diskhoji-updatecheck"),
                                        if auto { "on" } else { "off" },
                                    );
                                    if auto {
                                        recheck = true; // check now, since they just turned it on
                                    }
                                }
                                ui.label(
                                    RichText::new(
                                        "Off by default — with this unchecked, Diskhoji makes \
                                         no network connection at all.",
                                    )
                                    .size(11.5)
                                    .color(t.ink3),
                                );
                            });

                        ui.add_space(16.0);
                        // ----- credits -----
                        egui::Grid::new("about_grid")
                            .num_columns(2)
                            .spacing([16.0, 8.0])
                            .show(ui, |ui| {
                                ui.label(RichText::new("CREATED BY").color(t.ink3).size(12.0));
                                // our own opener: egui's hyperlink_to depends on
                                // eframe's URL hook, which does nothing on some desktops
                                let link = |ui: &mut egui::Ui, text: RichText, url: &str| {
                                    if ui.link(text.color(t.acc).underline()).on_hover_text(url).clicked() {
                                        crate::open_link(url);
                                    }
                                };
                                link(ui, RichText::new("Prateek Singh").size(14.0).strong(), "https://theaivibe.org/about");
                                ui.end_row();
                                ui.label(RichText::new("WEBSITE").color(t.ink3).size(12.0));
                                link(ui, RichText::new("diskhoji.org").size(14.0), "https://diskhoji.org");
                                ui.end_row();
                                ui.label(RichText::new("SOURCE").color(t.ink3).size(12.0));
                                link(ui, RichText::new("github.com/singhpratech/diskhoji").size(14.0), "https://github.com/singhpratech/diskhoji");
                                ui.end_row();
                                ui.label(RichText::new("LICENSE").color(t.ink3).size(12.0));
                                ui.label(RichText::new("MIT").size(14.0).color(t.ink2));
                                ui.end_row();
                            });

                        ui.add_space(18.0);
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.button("Close").clicked() {
                                self.show_about = false;
                            }
                        });
                    });
            });
        if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
            self.show_about = false;
        }
        if recheck {
            spawn_update_check(self.update.clone());
        }
    }

    fn game_window(&mut self, ctx: &egui::Context, t: &Theme) {
        if !self.show_game {
            return;
        }
        let screen = ctx.screen_rect();
        egui::Area::new(egui::Id::new("game_dim"))
            .fixed_pos(screen.min)
            .order(egui::Order::Middle)
            .show(ctx, |ui| {
                ui.painter().rect_filled(screen, 0.0, Color32::from_black_alpha(150));
            });

        let canvas_w = (screen.width() - 90.0).clamp(320.0, crate::voyage::W);
        let canvas_h = canvas_w * (crate::voyage::H / crate::voyage::W);
        let mut close = false;
        let mut best_changed = false;
        egui::Window::new("voyage")
            .title_bar(false)
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .frame(
                egui::Frame::default()
                    .fill(t.panel2)
                    .stroke(Stroke::new(1.0, t.line))
                    .rounding(12.0)
                    .inner_margin(egui::Margin::same(14.0)),
            )
            .show(ctx, |ui| {
                ui.set_width(canvas_w);
                ui.horizontal(|ui| {
                    ui.label(
                        RichText::new("The Seeker's Voyage")
                            .size(17.0)
                            .strong()
                            .color(t.acc),
                    );
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.button("✕ Close").clicked() {
                            close = true;
                        }
                        ui.add_space(6.0);
                        ui.label(
                            RichText::new(format!("BEST {}", self.voyage.best))
                                .size(13.0)
                                .color(t.ink3),
                        );
                        ui.add_space(10.0);
                        ui.label(
                            RichText::new(format!("AMBER BYTES {}", self.voyage.bytes))
                                .size(13.0)
                                .color(t.ink3),
                        );
                        ui.add_space(10.0);
                        ui.label(
                            RichText::new(format!("FATHOMS {}", self.voyage.fathoms()))
                                .size(13.0)
                                .color(t.ink3),
                        );
                    });
                });
                ui.add_space(8.0);
                let rect = Rect::from_min_size(
                    ui.cursor().min,
                    Vec2::new(canvas_w, canvas_h),
                );
                best_changed = self.voyage.frame(ui, rect);
                ui.add_space(6.0);
                ui.label(
                    RichText::new(
                        "Space, ↑, or click to jump — release early for a short hop, \
                         ↓ to dive. Esc closes.",
                    )
                    .size(12.5)
                    .color(t.ink3),
                );
            });
        if best_changed {
            let _ = std::fs::create_dir_all(dirs_config());
            let _ = std::fs::write(
                dirs_config().join("diskhoji-voyage-best"),
                self.voyage.best.to_string(),
            );
        }
        if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
            close = true;
        }
        if close {
            self.voyage.anchor();
            self.show_game = false;
        }
    }

    fn elevate_window(&mut self, ctx: &egui::Context, t: &Theme) {
        if !self.show_elevate {
            return;
        }
        // resolve an in-flight relaunch attempt (unix: the auth dialog child)
        if let Some(child) = &mut self.elevate_child {
            ctx.request_repaint_after(std::time::Duration::from_millis(250));
            match child.try_wait() {
                Ok(None) => {} // auth dialog still up, or the elevated app is starting
                Ok(Some(st)) => {
                    self.elevate_child = None;
                    if st.success() {
                        // the elevated instance normally retires us via
                        // --takeover before we get here; a clean exit means
                        // it already ran its course, so just leave
                        std::process::exit(0);
                    }
                    #[cfg(target_os = "linux")]
                    crate::elevate::revoke_root_x_access();
                    self.elevate_err = Some(crate::elevate::describe_failure(&st));
                }
                Err(_) => {
                    self.elevate_child = None;
                    #[cfg(target_os = "linux")]
                    crate::elevate::revoke_root_x_access();
                    self.elevate_err = Some("Lost track of the elevated process.".into());
                }
            }
        }
        let waiting = self.elevate_child.is_some();
        let screen = ctx.screen_rect();
        egui::Area::new(egui::Id::new("elevate_dim"))
            .fixed_pos(screen.min)
            .order(egui::Order::Middle)
            .show(ctx, |ui| {
                ui.painter().rect_filled(screen, 0.0, Color32::from_black_alpha(150));
            });
        let mut restart = false;
        let mut not_now = false;
        egui::Window::new("elevate")
            .title_bar(false)
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .frame(
                egui::Frame::default()
                    .fill(t.panel2)
                    .stroke(Stroke::new(1.0, t.line))
                    .rounding(12.0)
                    .inner_margin(egui::Margin::symmetric(24.0, 20.0)),
            )
            .show(ctx, |ui| {
                ui.set_width(440.0);
                ui.label(
                    RichText::new("Measure every folder?").size(20.0).strong().color(t.acc),
                );
                ui.add_space(8.0);
                ui.label(
                    RichText::new(
                        "Diskhoji is running as a normal user, so system areas and \
                         other users' files can't be read — they show up as unreadable. \
                         Restarted with administrator rights, every folder on the disk \
                         can be measured.",
                    )
                    .size(13.5)
                    .color(t.ink2),
                );
                ui.add_space(6.0);
                ui.label(RichText::new(ELEVATE_HOW).size(12.5).color(t.ink3));
                if let Some(err) = &self.elevate_err {
                    ui.add_space(8.0);
                    ui.label(RichText::new(err).size(13.0).color(t.danger));
                }
                ui.add_space(14.0);
                if waiting {
                    ui.horizontal(|ui| {
                        ui.spinner();
                        ui.label(
                            RichText::new("Waiting for authorization…")
                                .size(13.5)
                                .color(t.ink2),
                        );
                        if ui.button("Cancel").clicked() {
                            if let Some(mut c) = self.elevate_child.take() {
                                let _ = c.kill();
                                let _ = c.wait();
                            }
                            #[cfg(target_os = "linux")]
                            crate::elevate::revoke_root_x_access();
                        }
                    });
                } else {
                    ui.horizontal(|ui| {
                        let dark_ink = if self.theme_kind == ThemeKind::Dark {
                            hex("#161006")
                        } else {
                            hex("#FFFFFF")
                        };
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new("Restart elevated").strong().color(dark_ink),
                                )
                                .fill(t.acc),
                            )
                            .clicked()
                        {
                            restart = true;
                        }
                        if ui.button("Not now").clicked() {
                            not_now = true;
                        }
                    });
                    ui.add_space(10.0);
                    if ui
                        .checkbox(
                            &mut self.elevate_never,
                            RichText::new("Don't ask again when Diskhoji starts")
                                .size(12.5)
                                .color(t.ink2),
                        )
                        .changed()
                    {
                        let _ = std::fs::create_dir_all(dirs_config());
                        let _ = std::fs::write(
                            dirs_config().join("diskhoji-elevate"),
                            if self.elevate_never { "never" } else { "ask" },
                        );
                    }
                }
            });
        if restart {
            self.elevate_err = None;
            #[cfg(unix)]
            match crate::elevate::relaunch_elevated() {
                Ok(child) => self.elevate_child = Some(child),
                Err(e) => self.elevate_err = Some(e),
            }
            #[cfg(windows)]
            match crate::elevate::relaunch_elevated() {
                Ok(()) => std::process::exit(0),
                Err(e) => self.elevate_err = Some(e),
            }
        }
        if not_now || (!waiting && ctx.input(|i| i.key_pressed(egui::Key::Escape))) {
            self.show_elevate = false;
        }
    }

    fn toast(&mut self, msg: impl Into<String>, bad: bool) {
        self.toast = Some((msg.into(), Instant::now(), bad));
    }

    fn invalidate_map(&mut self) {
        self.map_key = (u64::MAX, 0, 0, 0, 0);
    }

    fn snapshot(&mut self) {
        let core = self.core.clone();
        let guard = core.store.read().unwrap();
        let Some(s) = guard.as_ref() else { return };
        let (disk_total, disk_free) = store_disk_usage(s);
        let mut ext_ids: Vec<usize> =
            (0..s.exts.len()).filter(|e| s.exts[*e].bytes > 0).collect();
        ext_ids.sort_unstable_by(|a, b| s.exts[*b].bytes.cmp(&s.exts[*a].bytes));
        ext_ids.truncate(40);
        let exts = ext_ids
            .iter()
            .map(|e| ExtRow {
                name: s.exts[*e].name.clone(),
                desc: scan::ext_description(&s.exts[*e].name),
                bytes: s.exts[*e].bytes,
                files: s.exts[*e].files,
                slot: s.exts[*e].slot,
            })
            .collect();
        let largest = s
            .largest
            .iter()
            .filter(|id| s.nodes[**id as usize].alive)
            .map(|id| {
                let n = &s.nodes[*id as usize];
                BigRow {
                    id: *id,
                    name: n.name.to_string(),
                    size: n.size,
                    slot: s.exts[n.ext as usize].slot,
                }
            })
            .collect();
        self.snap = Some(Snap {
            root: s.root_path.clone(),
            bytes: s.nodes[0].size,
            files: s.nodes[0].files as u64,
            dirs: s.dirs,
            errors: s.errors,
            skipped: s.skipped,
            elapsed: s.elapsed_ms,
            disk_total,
            disk_free,
            multi: s.multi,
            volumes: s.volumes.len(),
            exts,
            largest,
        });
        self.gen_seen = s.generation;
    }

    fn reset_view_state(&mut self, root: String) {
        self.zoom = 0;
        self.sel = None;
        self.sel_multi.clear();
        self.ext_sel = None;
        // node ids are arena indices reused across scans — anything holding
        // an id from the old generation must die with it
        self.delete_target = None;
        self.ctx_target = None;
        self.props = None;
        self.search_hits.clear();
        self.search_gen = 0;
        self.visible_rows.clear();
        self.crumbs = vec![(0, root)];
        self.invalidate_map();
    }

    fn save_prefs(&self) {
        let _ = std::fs::create_dir_all(dirs_config());
        if let Ok(s) = serde_json::to_string_pretty(&self.prefs) {
            let _ = std::fs::write(&self.prefs_json, s);
        }
    }

    fn save_scan_opts(&self) {
        let o = ScanOptions {
            skip_hidden: self.skip_hidden,
            excludes: ScanOptions::parse_excludes(&self.excludes_text),
        };
        crate::save_opts(&o);
        *self.core.opts.write().unwrap() = o;
    }

    /// All selected ids (multi-selection, or just the primary one).
    fn selection(&self) -> Vec<u32> {
        if self.sel_multi.len() > 1 {
            self.sel_multi.iter().copied().collect()
        } else {
            self.sel.into_iter().collect()
        }
    }

    fn refresh_search(&mut self, store: &Store) {
        if self.search.trim().is_empty() {
            self.search_hits.clear();
            self.search_gen = store.generation;
            self.search_zoom = self.zoom;
            return;
        }
        self.search_hits = scan::search(store, self.zoom, self.search.trim(), 1000);
        self.search_gen = store.generation;
        self.search_zoom = self.zoom;
    }

    fn export_csv(&mut self, id: u32) {
        let core = self.core.clone();
        let guard = core.store.read().unwrap();
        let Some(s) = guard.as_ref() else { return };
        if (id as usize) >= s.nodes.len() {
            return;
        }
        let base = std::env::var("HOME")
            .or_else(|_| std::env::var("USERPROFILE"))
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("."));
        let dl = base.join("Downloads");
        let dir = if dl.is_dir() { dl } else { base };
        let stamp = now_secs();
        let name = format!("diskhoji-{}-{}.csv", crate::safe_name(&scan::path_of(s, id)), stamp);
        let out = dir.join(name);
        let res = std::fs::File::create(&out).and_then(|f| {
            let mut w = std::io::BufWriter::new(f);
            scan::export_csv(s, id, &mut w)
        });
        drop(guard);
        match res {
            Ok(rows) => self.toast(format!("Exported {} rows → {}", fmt_n(rows), out.display()), false),
            Err(e) => self.toast(format!("export failed: {}", e), true),
        }
    }

    fn rebuild_crumbs(&mut self, store: &Store) {
        let mut c: Vec<(u32, String)> = scan::ancestors_of(store, self.zoom)
            .into_iter()
            .map(|a| {
                let name = if a == 0 {
                    store.root_path.clone()
                } else {
                    store.nodes[a as usize].name.to_string()
                };
                (a, name)
            })
            .collect();
        let zname = if self.zoom == 0 {
            store.root_path.clone()
        } else {
            store.nodes[self.zoom as usize].name.to_string()
        };
        c.push((self.zoom, zname));
        self.crumbs = c;
    }

    fn relayout(&mut self, store: &Store, w: f32, h: f32) {
        let flags = self.view.code()
            | (self.prefs.show_free as u8) << 2
            | (self.prefs.show_unknown as u8) << 3
            | (self.prefs.color_mode as u8) << 4
            | (self.prefs.cushions as u8) << 6
            | (self.prefs.outlines as u8) << 7;
        let key = (store.generation, self.zoom, w as u32, h as u32, flags);
        if key == self.map_key {
            return;
        }
        self.map_key = key;
        match self.view {
            View::Map => {
                let extras = treemap_extras(store, self.zoom, self.prefs.show_free, self.prefs.show_unknown);
                let (r, d) = treemap::layout(store, self.zoom, w as f64, h as f64, &extras);
                self.rects = r;
                self.dir_rects = d;
                self.build_map_cache(store);
            }
            View::Heat => self.build_heat(store, w, h),
            View::Rings => self.build_rings(store, w, h),
        }
    }

    /// The cushion mesh is expensive (40k rects → 160k vertices) — build it
    /// once per (generation, zoom, size, view, theme) and clone per frame.
    /// Colour of one map block under the current colour mode.
    fn block_color(&self, t: &Theme, r: &TRect, folder_rank: &std::collections::HashMap<u32, u8>) -> Color32 {
        if r.d >= 3 || r.s == SLOT_SMALL || (self.prefs.color_mode == ColorMode::Type) {
            return t.slot_color(r.s);
        }
        match self.prefs.color_mode {
            ColorMode::Folder => match folder_rank.get(&r.t) {
                Some(k) if *k < 8 => t.slots[*k as usize],
                _ => t.other,
            },
            ColorMode::Age => {
                let b = age_bucket(self.legend_now, r.m);
                age_color(t, b)
            }
            ColorMode::Type => t.slot_color(r.s),
        }
    }

    fn build_map_cache(&mut self, store: &Store) {
        let t = theme(self.theme_kind);
        self.legend_now = now_secs();
        // colour-by-folder: the zoom root's children ranked by size
        let mut folder_rank: std::collections::HashMap<u32, u8> = std::collections::HashMap::new();
        if self.prefs.color_mode == ColorMode::Folder {
            for (i, c) in sorted_children(store, self.zoom, SortKey::Size, true).into_iter().enumerate() {
                folder_rank.insert(c, i.min(255) as u8);
            }
        }
        let mut mesh = egui::Mesh::default();
        for r in &self.rects {
            let (x, y, w, h) = (r.x, r.y, r.w.max(0.6), r.h.max(0.6));
            let c = self.block_color(&t, r, &folder_rank);
            let (tl, br) = if self.prefs.cushions && r.d < 3 {
                (lighten(c, 0.20), darken(c, 0.26))
            } else {
                (c, c)
            };
            let i0 = mesh.vertices.len() as u32;
            mesh.colored_vertex(Pos2::new(x, y), tl);
            mesh.colored_vertex(Pos2::new(x + w, y), c);
            mesh.colored_vertex(Pos2::new(x + w, y + h), br);
            mesh.colored_vertex(Pos2::new(x, y + h), c);
            mesh.add_triangle(i0, i0 + 1, i0 + 2);
            mesh.add_triangle(i0, i0 + 2, i0 + 3);
        }
        self.cached_mesh = mesh;

        self.cached_labels.clear();
        if !self.prefs.labels {
            return;
        }
        let mut placed: Vec<Rect> = Vec::new();
        let mut fits = |est: Rect| {
            if placed.iter().any(|b| b.intersects(est)) {
                false
            } else {
                placed.push(est);
                true
            }
        };
        // free / unknown pseudo blocks carry their own label
        for r in &self.rects {
            if r.d < 3 || r.w < 70.0 || r.h < 24.0 {
                continue;
            }
            let text = truncate_to(&format!("{} · {}", r.n, fmt_bytes(r.z)), r.w - 14.0, 13.0);
            let pos = Pos2::new(r.x + 6.0, r.y + 5.0);
            let est = Rect::from_min_size(pos, Vec2::new(text.len() as f32 * 8.2, 17.0));
            if fits(est) {
                self.cached_labels.push((pos, text, true));
            }
        }
        for d in &self.dir_rects {
            if d.w < 110.0 || d.h < 30.0 {
                continue;
            }
            let text = truncate_to(&format!("{}/", d.n), d.w - 14.0, 13.0);
            let pos = Pos2::new(d.x + 6.0, d.y + 5.0);
            let est = Rect::from_min_size(pos, Vec2::new(text.len() as f32 * 8.2, 17.0));
            if fits(est) {
                self.cached_labels.push((pos, text, true));
            }
        }
        for r in &self.rects {
            if r.d != 0 || r.w < 130.0 || r.h < 40.0 {
                continue;
            }
            let text = truncate_to(&r.n, r.w - 14.0, 13.0);
            let pos = Pos2::new(r.x + 6.0, r.y + r.h - 22.0);
            let est = Rect::from_min_size(pos, Vec2::new(text.len() as f32 * 8.2, 17.0));
            if fits(est) {
                self.cached_labels.push((pos, text, false));
            }
        }
    }

    fn build_rings(&mut self, store: &Store, w: f32, h: f32) {
        self.sectors.clear();
        let max_r = (w.min(h) / 2.0 - 24.0).max(60.0);
        let hole = max_r * 0.22;
        const DEPTH: usize = 5;
        let thick = (max_r - hole) / DEPTH as f32;
        const TAU: f32 = std::f32::consts::TAU;

        fn rec(
            store: &Store,
            sectors: &mut Vec<Sector>,
            id: u32,
            depth: usize,
            a0: f32,
            a1: f32,
            hole: f32,
            thick: f32,
        ) {
            if depth >= 5 {
                return;
            }
            let n = &store.nodes[id as usize];
            let mut kids: Vec<u32> = (n.first_child..n.first_child + n.child_count)
                .filter(|c| store.nodes[*c as usize].alive && store.nodes[*c as usize].size > 0)
                .collect();
            if kids.is_empty() {
                return;
            }
            kids.sort_unstable_by(|a, b| {
                store.nodes[*b as usize].size.cmp(&store.nodes[*a as usize].size)
            });
            let total = n.size.max(1) as f64;
            let r0 = hole + depth as f32 * thick;
            let r1 = r0 + thick - 2.0;
            let span_all = a1 - a0;
            const MIN_SPAN: f32 = 0.008;
            let mut a = a0;
            for (ix, c) in kids.iter().enumerate() {
                let k = &store.nodes[*c as usize];
                let span = (k.size as f64 / total) as f32 * span_all;
                if span < MIN_SPAN {
                    let rest: u64 =
                        kids[ix..].iter().map(|c| store.nodes[*c as usize].size).sum();
                    let rspan = (rest as f64 / total) as f32 * span_all;
                    if rspan > 0.002 {
                        sectors.push(Sector {
                            id,
                            r0,
                            r1,
                            a0: a,
                            a1: a + rspan,
                            slot: SLOT_SMALL,
                            d: 2,
                            name: format!("{} small items", kids.len() - ix),
                            size: rest,
                        });
                    }
                    break;
                }
                let slot = if k.is_dir {
                    SLOT_DIRAGG
                } else {
                    store.exts[k.ext as usize].slot
                };
                sectors.push(Sector {
                    id: *c,
                    r0,
                    r1,
                    a0: a,
                    a1: a + span,
                    slot,
                    d: k.is_dir as u8,
                    name: k.name.to_string(),
                    size: k.size,
                });
                if k.is_dir && span > 0.02 {
                    rec(store, sectors, *c, depth + 1, a, a + span, hole, thick);
                }
                a += span;
            }
        }
        rec(store, &mut self.sectors, self.zoom, 0, 0.0, TAU, hole, thick);
        let zn = &store.nodes[self.zoom as usize];
        let zname = if self.zoom == 0 {
            store.root_path.clone()
        } else {
            zn.name.to_string()
        };
        self.ring_label = (zname, fmt_bytes(zn.size));
    }

    fn build_heat(&mut self, store: &Store, w: f32, h: f32) {
        self.heat_cells.clear();
        let n = &store.nodes[self.zoom as usize];
        let mut kids: Vec<u32> = (n.first_child..n.first_child + n.child_count)
            .filter(|c| store.nodes[*c as usize].alive && store.nodes[*c as usize].size > 0)
            .collect();
        kids.sort_unstable_by(|a, b| {
            store.nodes[*b as usize].size.cmp(&store.nodes[*a as usize].size)
        });
        self.heat_more = kids.len().saturating_sub(2000) as u32;
        kids.truncate(2000);
        if kids.is_empty() {
            return;
        }
        let pad = 20.0_f32;
        let leg = 40.0_f32;
        let aw = (w - pad * 2.0).max(40.0);
        let ah = (h - pad * 2.0 - leg).max(40.0);
        let mut pitch = ((aw * ah) / kids.len() as f32).sqrt().floor().clamp(10.0, 46.0);
        while pitch > 10.0
            && (kids.len() as f32 / (aw / pitch).floor().max(1.0)).ceil() * pitch > ah
        {
            pitch -= 1.0;
        }
        let cols = (aw / pitch).floor().max(1.0) as usize;
        let gap = (pitch * 0.18).max(2.0);
        let max_lg = (1.0 + store.nodes[kids[0] as usize].size as f64).log2();
        let min_lg = (1.0 + store.nodes[*kids.last().unwrap() as usize].size as f64).log2();
        for (ix, id) in kids.iter().enumerate() {
            let k = &store.nodes[*id as usize];
            let lg = (1.0 + k.size as f64).log2();
            let lvl = if max_lg > min_lg {
                ((lg - min_lg) / (max_lg - min_lg) * 4.999).floor().min(4.0) as u8
            } else {
                4
            };
            let x = pad + (ix % cols) as f32 * pitch;
            let y = pad + (ix / cols) as f32 * pitch;
            self.heat_cells.push(HeatCell {
                id: *id,
                rect: Rect::from_min_size(Pos2::new(x, y), Vec2::splat(pitch - gap)),
                lvl,
                slot: if k.is_dir {
                    SLOT_DIRAGG
                } else {
                    store.exts[k.ext as usize].slot
                },
                dir: k.is_dir,
                name: k.name.to_string(),
                size: k.size,
            });
        }
    }

    fn begin_scan(&mut self, target: ScanTarget) {
        match &target {
            ScanTarget::Path(p) => {
                if !p.is_dir() {
                    self.toast(format!("not a directory: {}", p.display()), true);
                    return;
                }
                self.path_input = p.to_string_lossy().into_owned();
            }
            ScanTarget::AllVolumes => self.path_input = scan::ALL_VOLUMES.to_string(),
        }
        self.scan_started = Some(Instant::now());
        start_scan(self.core.clone(), target);
    }

    fn rescan_last(&mut self) {
        let t = self.core.last_target.lock().unwrap().clone();
        if let Some(t) = t {
            self.begin_scan(t);
        }
    }

    fn path_for(&self, id: u32) -> Option<String> {
        let core = self.core.clone();
        let guard = core.store.read().unwrap();
        guard.as_ref().and_then(|s| {
            if (id as usize) < s.nodes.len() {
                Some(scan::path_of(s, id))
            } else {
                None
            }
        })
    }

    fn apply(&mut self, a: Act) {
        match a {
            Act::Zoom(id) => {
                // zooming into a leaf file gives a dead view — go to its parent
                let core = self.core.clone();
                let guard = core.store.read().unwrap();
                let target = guard
                    .as_ref()
                    .map(|s| {
                        if (id as usize) < s.nodes.len() && !s.nodes[id as usize].is_dir {
                            s.nodes[id as usize].parent
                        } else {
                            id
                        }
                    })
                    .unwrap_or(id);
                drop(guard);
                self.zoom = target;
                self.invalidate_map();
                self.crumbs.clear();
            }
            Act::ZoomParent(id) => {
                let core = self.core.clone();
                let guard = core.store.read().unwrap();
                let parent = guard
                    .as_ref()
                    .filter(|s| (id as usize) < s.nodes.len())
                    .map(|s| s.nodes[id as usize].parent)
                    .unwrap_or(0);
                drop(guard);
                self.zoom = parent;
                self.invalidate_map();
                self.crumbs.clear();
            }
            Act::Select(id) => {
                self.sel = Some(id);
                self.sel_multi.clear();
                self.sel_multi.insert(id);
            }
            Act::ToggleSelect(id) => {
                if self.sel_multi.contains(&id) {
                    self.sel_multi.remove(&id);
                    self.sel = self.sel_multi.iter().next_back().copied();
                } else {
                    if let Some(p) = self.sel {
                        self.sel_multi.insert(p);
                    }
                    self.sel_multi.insert(id);
                    self.sel = Some(id);
                }
            }
            Act::Locate(id) => {
                let core = self.core.clone();
                let guard = core.store.read().unwrap();
                if let Some(s) = guard.as_ref() {
                    if (id as usize) < s.nodes.len() {
                        self.expand_req = scan::ancestors_of(s, id);
                        self.scroll_to = Some(id);
                        self.search.clear();
                        self.search_hits.clear();
                    }
                }
                drop(guard);
                self.sel = Some(id);
                self.sel_multi.clear();
                self.sel_multi.insert(id);
            }
            Act::ToggleExt(s) => {
                self.ext_sel = if self.ext_sel == Some(s) { None } else { Some(s) };
            }
            Act::AskDelete(id) => {
                let targets = self.del_targets(&[id]);
                if !targets.is_empty() {
                    self.delete_target = Some(targets);
                }
            }
            Act::AskDeleteSelected => {
                let ids = self.selection();
                let targets = self.del_targets(&ids);
                if !targets.is_empty() {
                    self.delete_target = Some(targets);
                }
            }
            Act::Terminal(id) => {
                if let Some(p) = self.path_for(id) {
                    open_terminal(&p);
                }
            }
            Act::Props(id) => {
                let core = self.core.clone();
                let guard = core.store.read().unwrap();
                if let Some(s) = guard.as_ref() {
                    if (id as usize) < s.nodes.len() && s.nodes[id as usize].alive {
                        let n = &s.nodes[id as usize];
                        let parent = if id == 0 { n.size } else { s.nodes[n.parent as usize].size };
                        self.props = Some(Props {
                            name: n.name.to_string(),
                            path: scan::path_of(s, id),
                            dir: n.is_dir,
                            size: n.size,
                            files: n.files,
                            subdirs: n.subdirs,
                            mtime: n.mtime,
                            pct_parent: 100.0 * n.size as f64 / parent.max(1) as f64,
                            pct_total: 100.0 * n.size as f64 / s.nodes[0].size.max(1) as f64,
                            ext: if n.is_dir { String::new() } else { s.exts[n.ext as usize].name.clone() },
                        });
                    }
                }
            }
            Act::RescanFolder(id) => {
                let (gen, path) = {
                    let core = self.core.clone();
                    let guard = core.store.read().unwrap();
                    match guard.as_ref() {
                        Some(s) if (id as usize) < s.nodes.len() => (s.generation, scan::path_of(s, id)),
                        _ => return,
                    }
                };
                if id == 0 {
                    self.rescan_last();
                    return;
                }
                // keep the user where they are once the fresh tree lands
                self.restore_zoom = Some(self.path_for(self.zoom).unwrap_or(path.clone()));
                self.scan_started = Some(Instant::now());
                self.path_input = path;
                if let Err(e) = crate::rescan_subtree(self.core.clone(), id, gen) {
                    self.restore_zoom = None;
                    self.toast(e, true);
                }
            }
            Act::ExportCsv(id) => self.export_csv(id),
            Act::Reveal(id) => {
                if let Some(p) = self.path_for(id) {
                    reveal_in_file_manager(&p);
                }
            }
            Act::Open(id) => {
                if let Some(p) = self.path_for(id) {
                    open_with_default(&p);
                }
            }
            Act::CopyPath(id) => {
                self.pending_copy = self.path_for(id);
            }
        }
    }

    /// Build delete targets for a set of ids, dropping protected roots and
    /// anything nested inside another chosen item (its parent takes it).
    fn del_targets(&self, ids: &[u32]) -> Vec<DelTarget> {
        let core = self.core.clone();
        let guard = core.store.read().unwrap();
        let Some(s) = guard.as_ref() else { return Vec::new() };
        let valid: Vec<u32> = ids
            .iter()
            .copied()
            .filter(|id| (*id as usize) < s.nodes.len() && s.nodes[*id as usize].alive && !scan::is_protected(s, *id))
            .collect();
        let set: BTreeSet<u32> = valid.iter().copied().collect();
        valid
            .into_iter()
            .filter(|id| !scan::ancestors_of(s, *id).iter().any(|a| set.contains(a)))
            .map(|id| {
                let n = &s.nodes[id as usize];
                DelTarget {
                    id,
                    gen: s.generation,
                    name: n.name.to_string(),
                    path: scan::path_of(s, id),
                    size: n.size,
                    dir: n.is_dir,
                    files: n.files,
                }
            })
            .collect()
    }

    /// Validate against the generation captured when the modal opened, then
    /// run the filesystem removals on a worker thread — never on the UI
    /// thread, never while holding the store lock.
    fn do_delete(&mut self, targets: Vec<DelTarget>, to_trash: bool) {
        {
            let core = self.core.clone();
            let guard = core.store.read().unwrap();
            let Some(s) = guard.as_ref() else { return };
            for tgt in &targets {
                if s.generation != tgt.gen {
                    self.toast("scan changed underneath — delete cancelled", true);
                    return;
                }
                if scan::is_protected(s, tgt.id)
                    || tgt.id as usize >= s.nodes.len()
                    || !s.nodes[tgt.id as usize].alive
                    || scan::path_of(s, tgt.id) != tgt.path
                {
                    self.toast("refusing: delete target no longer valid", true);
                    return;
                }
            }
        }
        let slot: Arc<std::sync::Mutex<Option<Vec<(DelTarget, std::io::Result<()>)>>>> =
            Arc::new(std::sync::Mutex::new(None));
        let worker = slot.clone();
        let n = targets.len();
        std::thread::spawn(move || {
            let mut results = Vec::with_capacity(targets.len());
            for tgt in targets {
                let r = remove_path(&tgt.path, tgt.dir, to_trash);
                results.push((tgt, r));
            }
            if let Ok(mut g) = worker.lock() {
                *g = Some(results);
            }
        });
        self.pending_delete = Some((slot, n, to_trash));
    }

    fn poll_pending_delete(&mut self, ctx: &egui::Context) {
        let Some((slot, _, to_trash)) = &self.pending_delete else { return };
        let to_trash = *to_trash;
        ctx.request_repaint_after(std::time::Duration::from_millis(150));
        let done = slot.lock().ok().and_then(|mut g| g.take());
        let Some(results) = done else { return };
        self.pending_delete = None;
        let mut freed_total = 0u64;
        let mut ok = 0usize;
        let mut first_err: Option<String> = None;
        let mut last_name = String::new();
        {
            let core = self.core.clone();
            let mut guard = core.store.write().unwrap();
            for (tgt, r) in results {
                match r {
                    Err(e) => {
                        if first_err.is_none() {
                            first_err = Some(format!("{}: {}", tgt.name, e));
                        }
                    }
                    Ok(()) => {
                        ok += 1;
                        last_name = tgt.name.clone();
                        if let Some(s) = guard.as_mut() {
                            // the fs object is gone; only fix up the model if it
                            // still describes the generation we deleted from
                            if s.generation == tgt.gen
                                && (tgt.id as usize) < s.nodes.len()
                                && s.nodes[tgt.id as usize].alive
                            {
                                let (freed, _files) = scan::remove_subtree(s, tgt.id);
                                freed_total += freed;
                            }
                        }
                    }
                }
            }
            if let Some(s) = guard.as_ref() {
                while self.zoom != 0 && !s.nodes[self.zoom as usize].alive {
                    self.zoom = s.nodes[self.zoom as usize].parent;
                }
            }
        }
        if ok > 0 {
            self.sel = None;
            self.sel_multi.clear();
            self.deleted_since_scan = true;
            self.snapshot();
            self.invalidate_map();
            self.crumbs.clear();
            let verb = if to_trash { "Moved to trash" } else { "Deleted" };
            let what = if ok == 1 { last_name } else { format!("{} items", ok) };
            self.toast(format!("{} {} — {} freed", verb, what, fmt_bytes(freed_total)), false);
        }
        if let Some(e) = first_err {
            self.toast(format!("delete failed — {}", e), true);
        }
    }
}

// ---------- eframe app ----------

impl eframe::App for Native {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        let t = theme(self.theme_kind);
        let scanning = self.core.prog.scanning.load(Ordering::SeqCst);

        if !scanning {
            let gen = {
                let core = self.core.clone();
                let g = core.store.read().unwrap();
                g.as_ref().map(|s| s.generation).unwrap_or(0)
            };
            if gen != self.gen_seen && gen > 0 {
                self.snapshot();
                let root = self.snap.as_ref().map(|s| s.root.clone()).unwrap_or_default();
                self.reset_view_state(root);
                self.show_landing = false;
                self.deleted_since_scan = false;
                if let Some(p) = self.restore_zoom.take() {
                    let core = self.core.clone();
                    let g = core.store.read().unwrap();
                    if let Some(s) = g.as_ref() {
                        if let Some(id) = scan::find_by_path(s, &p) {
                            self.zoom = id;
                            self.crumbs.clear();
                        }
                    }
                }
            }
        }

        if scanning {
            ctx.request_repaint_after(std::time::Duration::from_millis(120));
            self.scanning_screen(ctx, &t);
        } else if self.show_landing || self.snap.is_none() {
            self.landing_screen(ctx, &t);
        } else {
            self.dashboard(ctx, &t);
        }
        // the game floats above whichever screen is showing
        self.game_window(ctx, &t);
        // ...and the elevation offer floats above everything
        self.elevate_window(ctx, &t);
    }
}

impl Native {
    // ----- landing -----
    fn landing_screen(&mut self, ctx: &egui::Context, t: &Theme) {
        let mut start: Option<String> = None;
        let mut start_all = false;
        egui::CentralPanel::default()
            .frame(egui::Frame::default().fill(t.bg))
            .show(ctx, |ui| {
                if self.picking {
                    ui.ctx()
                        .request_repaint_after(std::time::Duration::from_millis(200));
                    if let Ok(mut g) = self.picked.lock() {
                        if let Some(done) = g.take() {
                            self.picking = false;
                            if let Some(p) = done {
                                self.path_input = p;
                            }
                        }
                    }
                }
                if self.snap.is_some() && ui.button("← Back").clicked() {
                    self.show_landing = false;
                }
                ui.vertical_centered(|ui| {
                    ui.add_space(ui.available_height() * 0.14);
                    ui.horizontal(|ui| {
                        let total_w = 300.0;
                        ui.add_space((ui.available_width() - total_w).max(0.0) / 2.0);
                        if let Some(tex) = &self.logo {
                            ui.add(
                                egui::Image::new(tex)
                                    .fit_to_exact_size(Vec2::splat(52.0))
                                    .rounding(12.0),
                            );
                        }
                        ui.label(
                            RichText::new("Diskhoji").size(38.0).strong().color(t.acc),
                        );
                    });
                    ui.add_space(6.0);
                    ui.label(RichText::new("every byte, accounted for.").size(17.0).color(t.ink2));
                    ui.add_space(34.0);
                    let w = 660.0_f32.min(ui.available_width() - 40.0);
                    ui.allocate_ui(Vec2::new(w, ui.available_height()), |ui| {
                        ui.label(RichText::new("SCAN A FOLDER").size(13.0).color(t.ink3));
                        ui.add_space(6.0);
                        ui.horizontal(|ui| {
                            let te = egui::TextEdit::singleline(&mut self.path_input)
                                .desired_width(w - 300.0)
                                .font(TextStyle::Body);
                            let resp = ui.add(te);
                            if ui.button("Browse…").clicked() && !self.picking {
                                self.picking = true;
                                let slot = self.picked.clone();
                                std::thread::spawn(move || {
                                    let r = pick_folder_blocking();
                                    if let Ok(mut g) = slot.lock() {
                                        *g = Some(r);
                                    }
                                });
                            }
                            let dark_ink = if self.theme_kind == ThemeKind::Dark {
                                hex("#161006")
                            } else {
                                hex("#FFFFFF")
                            };
                            let go = ui.add(
                                egui::Button::new(RichText::new("Scan").strong().color(dark_ink))
                                    .fill(t.acc),
                            );
                            if go.clicked()
                                || (resp.lost_focus()
                                    && ui.input(|i| i.key_pressed(egui::Key::Enter)))
                            {
                                start = Some(self.path_input.clone());
                            }
                        });
                        ui.add_space(22.0);
                        ui.label(RichText::new("OR PICK A VOLUME").size(13.0).color(t.ink3));
                        ui.add_space(6.0);
                        {
                            // WinDirStat's "All local drives"
                            let nvol = crate::all_volumes().len();
                            let text = format!(
                                "All volumes\n{} local filesystem{} in one map",
                                nvol,
                                if nvol == 1 { "" } else { "s" }
                            );
                            let btn = egui::Button::new(RichText::new(text).size(14.5).strong())
                                .fill(t.panel2)
                                .stroke(Stroke::new(1.0, t.acc.gamma_multiply(0.6)));
                            if ui.add_sized(Vec2::new(ui.available_width(), 56.0), btn).clicked() {
                                start_all = true;
                            }
                            ui.add_space(4.0);
                        }
                        let roots = self.roots.clone();
                        for chunk in roots.chunks(2) {
                            ui.columns(2, |cols| {
                                for (i, r) in chunk.iter().enumerate() {
                                    let used = if r.total > 0 {
                                        (r.total - r.free) as f32 / r.total as f32
                                    } else {
                                        0.0
                                    };
                                    let text = format!(
                                        "{}\n{}\n{} free of {}",
                                        r.label,
                                        r.path,
                                        fmt_bytes(r.free),
                                        fmt_bytes(r.total)
                                    );
                                    let btn = egui::Button::new(RichText::new(text).size(14.5))
                                        .fill(t.panel)
                                        .stroke(Stroke::new(1.0, t.line));
                                    let resp = cols[i].add_sized(
                                        Vec2::new(cols[i].available_width(), 78.0),
                                        btn,
                                    );
                                    let bar = Rect::from_min_size(
                                        resp.rect.left_bottom() + Vec2::new(12.0, -13.0),
                                        Vec2::new(resp.rect.width() - 24.0, 5.0),
                                    );
                                    cols[i].painter().rect_filled(bar, 2.0, t.line);
                                    let mut fb = bar;
                                    fb.set_right(bar.left() + bar.width() * used);
                                    cols[i].painter().rect_filled(fb, 2.0, t.ink3);
                                    if resp.clicked() {
                                        start = Some(r.path.clone());
                                    }
                                }
                            });
                            ui.add_space(4.0);
                        }
                        ui.add_space(10.0);
                        let active = self.skip_hidden || !self.excludes_text.trim().is_empty();
                        let hdr = if active { "SCAN OPTIONS · filters active" } else { "SCAN OPTIONS" };
                        egui::CollapsingHeader::new(RichText::new(hdr).size(13.0).color(if active { t.acc } else { t.ink3 }))
                            .id_salt("scan_opts")
                            .default_open(self.show_opts || active)
                            .show(ui, |ui| {
                                let mut changed = false;
                                changed |= ui
                                    .checkbox(&mut self.skip_hidden, "Skip hidden files and folders")
                                    .on_hover_text("Dot-files on Linux/macOS, Hidden-attribute entries on Windows")
                                    .changed();
                                ui.horizontal(|ui| {
                                    ui.label(RichText::new("Exclude names:").size(14.0));
                                    let te = egui::TextEdit::singleline(&mut self.excludes_text)
                                        .hint_text("node_modules, *.iso, Cache*")
                                        .desired_width(ui.available_width() - 8.0)
                                        .font(TextStyle::Body);
                                    let r = ui.add(te).on_hover_text(
                                        "Comma-separated. * and ? wildcards. Matched against each file or folder name, case-insensitive.",
                                    );
                                    changed |= r.changed();
                                });
                                ui.label(
                                    RichText::new("Excluded items are left out of sizes and counted under FILES as \"excluded\".")
                                        .size(12.5)
                                        .color(t.ink3),
                                );
                                if changed {
                                    self.save_scan_opts();
                                }
                            });
                        ui.add_space(10.0);
                        ui.label(
                            RichText::new(
                                "Scanning stays on one filesystem, and Diskhoji makes no \
                                 network connection on its own — only if you opt into update checks.",
                            )
                            .size(13.5)
                            .color(t.ink3),
                        );
                        ui.add_space(6.0);
                        if self.elevated {
                            ui.label(
                                RichText::new(
                                    "Running with administrator rights — every folder \
                                     can be measured.",
                                )
                                .size(13.5)
                                .color(t.ink3),
                            );
                        } else if ui
                            .link(
                                RichText::new(
                                    "Restart with administrator rights — measure every folder",
                                )
                                .size(13.5),
                            )
                            .clicked()
                        {
                            self.elevate_err = None;
                            self.show_elevate = true;
                        }
                        ui.add_space(12.0);
                        if ui
                            .button("Play game — The Seeker's Voyage")
                            .on_hover_text("A little offline diversion while you decide what to scan")
                            .clicked()
                        {
                            self.show_game = true;
                        }
                    });
                });
            });
        if start_all {
            self.begin_scan(ScanTarget::AllVolumes);
        } else if let Some(p) = start {
            self.begin_scan(ScanTarget::Path(PathBuf::from(p)));
        }
        self.paint_toast(ctx, t);
    }

    // ----- scanning -----
    fn scanning_screen(&mut self, ctx: &egui::Context, t: &Theme) {
        let mut cancel = false;
        egui::CentralPanel::default()
            .frame(egui::Frame::default().fill(t.bg))
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(ui.available_height() * 0.24);
                    ui.label(RichText::new("● SCANNING").size(14.0).color(t.acc));
                    ui.add_space(4.0);
                    ui.label(RichText::new(&self.path_input).size(14.0).color(t.ink3));
                    ui.add_space(18.0);
                    let bytes = self.core.prog.bytes.load(Ordering::Relaxed);
                    ui.label(RichText::new(fmt_bytes(bytes)).size(54.0).strong());
                    ui.add_space(16.0);
                    let files = self.core.prog.files.load(Ordering::Relaxed);
                    let dirs = self.core.prog.dirs.load(Ordering::Relaxed);
                    let el = self
                        .scan_started
                        .map(|s| s.elapsed().as_secs_f32())
                        .unwrap_or(0.0);
                    ui.label(
                        RichText::new(format!(
                            "{} files   ·   {} folders   ·   {:.1} s",
                            fmt_n(files),
                            fmt_n(dirs),
                            el
                        ))
                        .size(18.0)
                        .color(t.ink2),
                    );
                    ui.add_space(10.0);
                    let cur = self.core.prog.current.lock().unwrap().clone();
                    ui.label(RichText::new(cur).size(13.0).color(t.ink3));
                    ui.add_space(22.0);
                    if ui.button("Cancel scan").clicked() {
                        cancel = true;
                    }
                });
            });
        if cancel {
            self.core.prog.cancel.store(true, Ordering::SeqCst);
        }
    }

    // ----- dashboard -----
    fn dashboard(&mut self, ctx: &egui::Context, t: &Theme) {
        // a persisted zoom from an older build could exceed the new cap
        let zf = ctx.zoom_factor();
        if zf > 1.6 || zf < 0.8 {
            ctx.set_zoom_factor(zf.clamp(0.8, 1.6));
        }
        self.poll_pending_delete(ctx);
        if let Some((_, n, to_trash)) = &self.pending_delete {
            let name = if *n == 1 {
                if *to_trash { "Moving to trash…".to_string() } else { "Deleting…".to_string() }
            } else if *to_trash {
                format!("Moving {} items to trash…", n)
            } else {
                format!("Deleting {} items…", n)
            };
            egui::Area::new(egui::Id::new("del_chip"))
                .anchor(Align2::CENTER_TOP, Vec2::new(0.0, 70.0))
                .order(egui::Order::Foreground)
                .show(ctx, |ui| {
                    egui::Frame::default()
                        .fill(t.panel2)
                        .stroke(Stroke::new(1.0, t.acc))
                        .rounding(7.0)
                        .inner_margin(egui::Margin::symmetric(14.0, 8.0))
                        .show(ui, |ui| {
                            ui.label(RichText::new(name).size(14.0));
                        });
                });
        }
        let core = self.core.clone();
        let mut acts: Vec<Act> = Vec::new();
        let mut rescan = false;
        let mut retarget = false;
        let mut theme_flip = false;

        let modal_up = self.delete_target.is_some()
            || self.props.is_some()
            || self.trash_confirm.is_some()
            || self.show_about
            || self.show_game;
        let typing = ctx.memory(|m| m.focused().is_some());
        if !modal_up && !typing {
            let (backspace, del, up, down, left, right, enter, f5, ctrl, c, o, f, e, a, t) =
                ctx.input(|i| {
                    (
                        i.key_pressed(egui::Key::Backspace),
                        i.key_pressed(egui::Key::Delete),
                        i.key_pressed(egui::Key::ArrowUp),
                        i.key_pressed(egui::Key::ArrowDown),
                        i.key_pressed(egui::Key::ArrowLeft),
                        i.key_pressed(egui::Key::ArrowRight),
                        i.key_pressed(egui::Key::Enter),
                        i.key_pressed(egui::Key::F5),
                        i.modifiers.command,
                        i.key_pressed(egui::Key::C),
                        i.key_pressed(egui::Key::O),
                        i.key_pressed(egui::Key::F),
                        i.key_pressed(egui::Key::E),
                        i.key_pressed(egui::Key::A),
                        i.key_pressed(egui::Key::T),
                    )
                });
            if backspace && self.crumbs.len() > 1 {
                acts.push(Act::Zoom(self.crumbs[self.crumbs.len() - 2].0));
            }
            if del && !self.selection().is_empty() {
                acts.push(Act::AskDeleteSelected);
            }
            if f5 {
                self.rescan_last();
            }
            if ctrl && f {
                self.focus_search = true;
            }
            if ctrl && e {
                acts.push(Act::ExportCsv(self.zoom));
            }
            if ctrl && a {
                // select every visible explorer row
                let rows = self.visible_rows.clone();
                if !rows.is_empty() {
                    self.sel_multi = rows.iter().copied().collect();
                    self.sel = rows.last().copied();
                }
            }
            if let Some(sel) = self.sel {
                if ctrl && c {
                    acts.push(Act::CopyPath(sel));
                }
                if ctrl && o {
                    acts.push(Act::Open(sel));
                }
                if ctrl && t {
                    acts.push(Act::Terminal(sel));
                }
                if enter {
                    acts.push(Act::Zoom(sel));
                }
                if right {
                    self.expand_req.push(sel);
                }
                if left {
                    // collapse if open, else step to the parent row
                    let cid = egui::Id::new((self.gen_seen, sel, "tree"));
                    let open = egui::collapsing_header::CollapsingState::load(ctx, cid)
                        .map(|st| st.is_open())
                        .unwrap_or(false);
                    if open {
                        if let Some(mut st) = egui::collapsing_header::CollapsingState::load(ctx, cid) {
                            st.set_open(false);
                            st.store(ctx);
                        }
                    } else {
                        let core = self.core.clone();
                        let g = core.store.read().unwrap();
                        if let Some(s) = g.as_ref() {
                            if sel != 0 && (sel as usize) < s.nodes.len() {
                                acts.push(Act::Select(s.nodes[sel as usize].parent));
                                self.scroll_to = Some(s.nodes[sel as usize].parent);
                            }
                        }
                    }
                }
                if up || down {
                    if let Some(pos) = self.visible_rows.iter().position(|r| *r == sel) {
                        let np = if up { pos.saturating_sub(1) } else { (pos + 1).min(self.visible_rows.len() - 1) };
                        let id = self.visible_rows[np];
                        acts.push(Act::Select(id));
                        self.scroll_to = Some(id);
                    }
                }
            } else if (up || down) && !self.visible_rows.is_empty() {
                let id = self.visible_rows[0];
                acts.push(Act::Select(id));
                self.scroll_to = Some(id);
            }
        }
        if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
            if self.delete_target.is_some() {
                self.delete_target = None;
            } else if self.props.is_some() {
                self.props = None;
            } else if self.trash_confirm.is_some() {
                self.trash_confirm = None;
            } else if !self.search.is_empty() && !typing {
                self.search.clear();
                self.search_hits.clear();
            } else if self.ext_sel.is_some() {
                self.ext_sel = None;
            } else if self.sel_multi.len() > 1 {
                self.sel_multi.clear();
                self.sel = None;
            }
        }

        // header
        egui::TopBottomPanel::top("header")
            .frame(
                egui::Frame::default()
                    .fill(t.panel)
                    .inner_margin(egui::Margin::symmetric(16.0, 10.0)),
            )
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    if let Some(tex) = &self.logo {
                        ui.add(
                            egui::Image::new(tex)
                                .fit_to_exact_size(Vec2::splat(26.0))
                                .rounding(6.0),
                        );
                    } else {
                        ui.label(RichText::new("▦").size(22.0).color(t.acc));
                    }
                    ui.label(RichText::new("Diskhoji").size(17.0).strong());
                    ui.label(
                        RichText::new(concat!("v", env!("CARGO_PKG_VERSION")))
                            .size(13.0)
                            .color(t.ink3),
                    );
                    ui.add_space(10.0);
                    // Right-side controls reserve space from the right edge; the
                    // path + action buttons then fill the middle, and the path
                    // truncates instead of colliding with the controls.
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        // Zoom controls FIRST so they pin to the right edge and
                        // don't drift under the cursor as the UI scales — pressing
                        // A+ repeatedly can no longer land on the theme toggle.
                        // Text size as a menu: you pick a discrete size once, so
                        // no repeatedly-clicked button that drifts under the cursor.
                        let z = ctx.zoom_factor();
                        ui.menu_button(format!("Text {:.0}%", z * 100.0), |ui| {
                            ui.set_min_width(150.0);
                            for (lbl, val) in [
                                ("Small — 90%", 0.9_f32),
                                ("Normal — 100%", 1.0),
                                ("Large — 120%", 1.2),
                                ("Larger — 140%", 1.4),
                            ] {
                                let on = (z - val).abs() < 0.03;
                                if ui.selectable_label(on, lbl).clicked() {
                                    ctx.set_zoom_factor(val);
                                    ui.close_menu();
                                }
                            }
                        });
                        let label = match self.theme_kind {
                            ThemeKind::Dark => "Light",
                            ThemeKind::Light => "Dark",
                        };
                        if ui.button(label).clicked() {
                            theme_flip = true;
                        }
                        if ui.button("About").clicked() {
                            self.show_about = true;
                        }
                        if ui.button("Play game").clicked() {
                            self.show_game = true;
                        }
                        if let UpdateState::Available { ver, .. } =
                            self.update.lock().map(|g| g.clone()).unwrap_or(UpdateState::Failed)
                        {
                            let dark_ink = if self.theme_kind == ThemeKind::Dark {
                                hex("#161006")
                            } else {
                                hex("#FFFFFF")
                            };
                            if ui
                                .add(
                                    egui::Button::new(
                                        RichText::new(format!("Update {}", ver))
                                            .strong()
                                            .color(dark_ink),
                                    )
                                    .fill(t.acc),
                                )
                                .on_hover_text("A newer version is available")
                                .clicked()
                            {
                                self.show_about = true;
                            }
                        }
                        ui.add_space(8.0);
                        // middle band, left-aligned, path shrinks to fit
                        ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                            if ui.button("⟳ Rescan").on_hover_text("Scan the same target again (F5)").clicked() {
                                rescan = true;
                            }
                            if ui.button("Change target").clicked() {
                                retarget = true;
                            }
                            ui.menu_button("Tools", |ui| {
                                ui.set_min_width(560.0);
                                if ui.button("⇩  Export CSV — whole scan").clicked() {
                                    acts.push(Act::ExportCsv(0));
                                    ui.close_menu();
                                }
                                if ui.button("⇩  Export CSV — this folder  (Ctrl+E)").clicked() {
                                    acts.push(Act::ExportCsv(self.zoom));
                                    ui.close_menu();
                                }
                                if ui.button("▸  Open terminal here  (Ctrl+T)").clicked() {
                                    acts.push(Act::Terminal(self.zoom));
                                    ui.close_menu();
                                }
                                ui.separator();
                                if ui.button("♻  Empty trash / recycle bin…").clicked() {
                                    let (n, _) = crate::trash_info();
                                    self.trash_confirm = Some(n);
                                    ui.close_menu();
                                }
                                ui.separator();
                                ui.label(RichText::new("KEYBOARD").size(11.5).color(t.ink3));
                                ui.label(
                                    RichText::new(
                                        "↑↓ move · ←→ collapse/expand · Enter zoom · Backspace up\n\
                                         Del delete · Ctrl+C copy path · Ctrl+O open · Ctrl+F find\n\
                                         Ctrl+A select all · Ctrl+click multi-select · F5 rescan",
                                    )
                                    .size(12.0)
                                    .color(t.ink2),
                                );
                            });
                            if let Some(s) = &self.snap {
                                ui.add_space(6.0);
                                ui.add(
                                    egui::Label::new(
                                        RichText::new(&s.root).size(14.0).color(t.ink2),
                                    )
                                    .wrap_mode(egui::TextWrapMode::Truncate),
                                )
                                .on_hover_text(&s.root);
                            }
                        });
                    });
                });
            });

        // stats
        egui::TopBottomPanel::top("stats")
            .frame(
                egui::Frame::default()
                    .fill(t.panel)
                    .inner_margin(egui::Margin::symmetric(16.0, 12.0)),
            )
            .show(ctx, |ui| {
                if let Some(s) = &self.snap {
                    let unreadable_hover = {
                        let v = core
                            .prog
                            .err_paths
                            .lock()
                            .map(|v| v.clone())
                            .unwrap_or_default();
                        if v.is_empty() {
                            String::new()
                        } else {
                            let mut h = String::from(
                                "Not readable by your user (permission denied):\n",
                            );
                            for p in v.iter().take(14) {
                                h.push_str("  ");
                                h.push_str(p);
                                h.push('\n');
                            }
                            if v.len() > 14 {
                                h.push_str("  … and more");
                            }
                            h
                        }
                    };
                    let used_pct = if s.disk_total > s.disk_free {
                        100.0 * s.bytes as f64 / (s.disk_total - s.disk_free) as f64
                    } else {
                        0.0
                    };
                    let tiles: [(&str, String, String); 6] = [
                        ("SCANNED", fmt_bytes(s.bytes), format!("{} of used disk", fmt_pct(used_pct))),
                        (
                            "FILES",
                            fmt_n(s.files),
                            match (s.errors > 0, s.skipped > 0) {
                                (true, true) => format!("{} unreadable · {} excluded", fmt_n(s.errors), fmt_n(s.skipped)),
                                (true, false) => format!("{} unreadable", fmt_n(s.errors)),
                                (false, true) => format!("{} excluded by filters", fmt_n(s.skipped)),
                                _ => "all readable".into(),
                            },
                        ),
                        (
                            "FOLDERS",
                            fmt_n(s.dirs),
                            if s.multi { format!("{} volumes", s.volumes) } else { "one filesystem".into() },
                        ),
                        (
                            "LARGEST FILE",
                            s.largest.first().map(|b| fmt_bytes(b.size)).unwrap_or("—".into()),
                            s.largest.first().map(|b| b.name.clone()).unwrap_or_default(),
                        ),
                        (
                            "SCAN TIME",
                            fmt_time(s.elapsed),
                            format!(
                                "{} files/s",
                                fmt_n(if s.elapsed > 0 { s.files * 1000 / s.elapsed } else { 0 })
                            ),
                        ),
                        (
                            "FREE SPACE",
                            fmt_bytes(s.disk_free),
                            format!(
                                "{} of {} free",
                                fmt_pct(100.0 * s.disk_free as f64 / s.disk_total.max(1) as f64),
                                fmt_bytes(s.disk_total)
                            ),
                        ),
                    ];
                    ui.columns(6, |cols| {
                        for (i, (lbl, val, sub)) in tiles.iter().enumerate() {
                            egui::Frame::default()
                                .fill(t.panel2)
                                .stroke(Stroke::new(1.0, t.line))
                                .rounding(8.0)
                                .inner_margin(egui::Margin::symmetric(14.0, 10.0))
                                .show(&mut cols[i], |ui| {
                                    ui.set_width(ui.available_width());
                                    ui.label(
                                        RichText::new(*lbl).size(12.5).color(t.ink3),
                                    );
                                    let vsize = (ui.available_width()
                                        / (val.chars().count().max(4) as f32 * 0.64))
                                        .clamp(15.0, 27.0);
                                    ui.label(RichText::new(val).size(vsize).strong());
                                    let hover = if i == 1 && !unreadable_hover.is_empty() {
                                        unreadable_hover.as_str()
                                    } else {
                                        sub.as_str()
                                    };
                                    ui.label(
                                        RichText::new(truncate_to(
                                            sub,
                                            ui.available_width(),
                                            13.0,
                                        ))
                                        .size(13.0)
                                        .color(t.ink3),
                                    )
                                    .on_hover_text(hover);
                                });
                        }
                    });
                }
            });

        // status bar
        egui::TopBottomPanel::bottom("status")
            .frame(
                egui::Frame::default()
                    .fill(t.panel)
                    .inner_margin(egui::Margin::symmetric(16.0, 8.0)),
            )
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    let msg = if self.sel_multi.len() > 1 {
                        let guard = core.store.read().unwrap();
                        guard
                            .as_ref()
                            .map(|s| {
                                let ids: Vec<u32> = self
                                    .sel_multi
                                    .iter()
                                    .copied()
                                    .filter(|i| (*i as usize) < s.nodes.len() && s.nodes[*i as usize].alive)
                                    .collect();
                                let bytes: u64 = ids.iter().map(|i| s.nodes[*i as usize].size).sum();
                                format!(
                                    "{} items selected · {} · {} of total · Del to delete, Esc to clear",
                                    ids.len(),
                                    fmt_bytes(bytes),
                                    fmt_pct(100.0 * bytes as f64 / s.nodes[0].size.max(1) as f64)
                                )
                            })
                            .unwrap_or_else(|| "ready".into())
                    } else if let Some(sel) = self.sel {
                        let guard = core.store.read().unwrap();
                        guard
                            .as_ref()
                            .filter(|s| (sel as usize) < s.nodes.len() && s.nodes[sel as usize].alive)
                            .map(|s| {
                                let n = &s.nodes[sel as usize];
                                format!(
                                    "{} · {} · {} of total · {}",
                                    scan::path_of(s, sel),
                                    fmt_bytes(n.size),
                                    fmt_pct(
                                        100.0 * n.size as f64 / s.nodes[0].size.max(1) as f64
                                    ),
                                    scan::fmt_date(n.mtime)
                                )
                            })
                            .unwrap_or_else(|| "ready".into())
                    } else {
                        "ready · dbl-click to zoom · right-click for actions · ctrl-click to multi-select".into()
                    };
                    let left_w = (ui.available_width() - 460.0).max(200.0);
                    ui.label(
                        RichText::new(truncate_to(&msg, left_w, 14.0))
                            .size(14.0)
                            .color(t.ink2),
                    )
                    .on_hover_text(&msg);
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if let Some(s) = &self.snap {
                            let free = if s.disk_total > 0 {
                                format!(" · {} free", fmt_bytes(s.disk_free))
                            } else {
                                String::new()
                            };
                            ui.label(
                                RichText::new(format!(
                                    "{} files · {} folders · {}{}",
                                    fmt_n(s.files),
                                    fmt_n(s.dirs),
                                    fmt_time(s.elapsed),
                                    free
                                ))
                                .size(13.0)
                                .color(t.ink3),
                            );
                        }
                    });
                });
            });

        // left tree
        egui::SidePanel::left("tree")
            .frame(
                egui::Frame::default()
                    .fill(t.panel)
                    .inner_margin(egui::Margin::symmetric(8.0, 8.0)),
            )
            .default_width(520.0)
            .width_range(320.0..=800.0)
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("EXPLORER").size(12.5).color(t.ink3));
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if !self.search.is_empty() && ui.small_button("✕").clicked() {
                            self.search.clear();
                            self.search_hits.clear();
                        }
                        let te = egui::TextEdit::singleline(&mut self.search)
                            .hint_text("find by name…  (Ctrl+F, * ? wildcards)")
                            .desired_width(ui.available_width() - 8.0)
                            .font(TextStyle::Small);
                        let resp = ui.add(te);
                        if self.focus_search {
                            resp.request_focus();
                            self.focus_search = false;
                        }
                        if resp.changed() {
                            self.search_gen = 0; // force refresh
                        }
                    });
                });
                ui.add_space(4.0);
                let guard = core.store.read().unwrap();
                if let Some(s) = guard.as_ref() {
                    if !self.search.trim().is_empty() {
                        if self.search_gen != s.generation || self.search_zoom != self.zoom {
                            self.refresh_search(s);
                        }
                        let hits = self.search_hits.clone();
                        let zoom_path = scan::path_of(s, self.zoom);
                        ui.label(
                            RichText::new(format!(
                                "{} match{} under {}{}",
                                fmt_n(hits.len() as u64),
                                if hits.len() == 1 { "" } else { "es" },
                                truncate_to(self.crumbs.last().map(|c| c.1.as_str()).unwrap_or(""), 160.0, 12.5),
                                if hits.len() >= 1000 { " · first 1000" } else { "" }
                            ))
                            .size(12.5)
                            .color(t.ink3),
                        );
                        ui.add_space(2.0);
                        self.visible_rows = hits.clone();
                        egui::ScrollArea::vertical()
                            .auto_shrink([false, false])
                            .show(ui, |ui| {
                                for id in hits.iter().take(1000) {
                                    let k = &s.nodes[*id as usize];
                                    if !k.alive {
                                        continue;
                                    }
                                    let chip = if k.is_dir { None } else { Some(t.slot_color(s.exts[k.ext as usize].slot)) };
                                    let full = scan::path_of(s, *id);
                                    let rel = full
                                        .strip_prefix(zoom_path.as_str())
                                        .map(|r| r.trim_start_matches(std::path::MAIN_SEPARATOR).to_string())
                                        .unwrap_or(full);
                                    let cols = TreeCols { items: false, date: true };
                                    tree_row(
                                        ui, t, &rel, 100.0 * k.size as f64 / s.nodes[self.zoom as usize].size.max(1) as f64,
                                        k.size, k.mtime, k.files as u64 + k.subdirs as u64, chip,
                                        self.sel_multi.contains(id) || self.sel == Some(*id),
                                        &mut acts, *id, k.is_dir, cols, true,
                                    );
                                }
                            });
                    } else {
                        let w = ui.available_width();
                        let cols = TreeCols { items: w >= 470.0, date: w >= 400.0 };
                        // column headers, click to sort
                        ui.horizontal(|ui| {
                            ui.spacing_mut().item_spacing.x = 0.0;
                            let mut hdr = |ui: &mut egui::Ui, label: &str, key: SortKey, width: f32, align_right: bool| {
                                let on = self.sort_key == key;
                                let arrow = if on { if self.sort_desc { " ▾" } else { " ▴" } } else { "" };
                                let text = RichText::new(format!("{}{}", label, arrow))
                                    .size(11.5)
                                    .color(if on { t.acc } else { t.ink3 });
                                let (rect, resp) = ui.allocate_exact_size(Vec2::new(width, 18.0), Sense::click());
                                let anchor = if align_right { Align2::RIGHT_CENTER } else { Align2::LEFT_CENTER };
                                let pos = if align_right { Pos2::new(rect.max.x - 4.0, rect.center().y) } else { Pos2::new(rect.min.x + 4.0, rect.center().y) };
                                ui.painter().text(pos, anchor, text.text(), FontId::new(11.5, FontFamily::Monospace), if on { t.acc } else { t.ink3 });
                                if resp.clicked() {
                                    if self.sort_key == key {
                                        self.sort_desc = !self.sort_desc;
                                    } else {
                                        self.sort_key = key;
                                        self.sort_desc = key != SortKey::Name;
                                    }
                                }
                                resp.on_hover_text("Click to sort");
                            };
                            let right_w = 80.0 + 50.0 + if cols.date { 92.0 } else { 0.0 } + if cols.items { 64.0 } else { 0.0 };
                            hdr(ui, "NAME", SortKey::Name, (w - right_w - 24.0).max(60.0), false);
                            if cols.items {
                                hdr(ui, "ITEMS", SortKey::Items, 64.0, true);
                            }
                            if cols.date {
                                hdr(ui, "MODIFIED", SortKey::Date, 92.0, true);
                            }
                            hdr(ui, "%", SortKey::Size, 50.0, true);
                            hdr(ui, "SIZE", SortKey::Size, 80.0, true);
                        });
                        self.visible_rows.clear();
                        let expand = std::mem::take(&mut self.expand_req);
                        for id in &expand {
                            let cid = egui::Id::new((s.generation, *id, "tree"));
                            let mut st = egui::collapsing_header::CollapsingState::load_with_default_open(ctx, cid, false);
                            st.set_open(true);
                            st.store(ctx);
                        }
                        let scroll_to = self.scroll_to.take();
                        let sel_set = self.sel_multi.clone();
                        let mut vis: Vec<u32> = Vec::new();
                        egui::ScrollArea::vertical()
                            .auto_shrink([false, false])
                            .show(ui, |ui| {
                                let mut tc = TreeCtx {
                                    sort: (self.sort_key, self.sort_desc),
                                    sel: self.sel,
                                    sel_set: &sel_set,
                                    scroll_to,
                                    cols,
                                    vis: &mut vis,
                                };
                                draw_tree(ui, s, 0, 0, t, &mut tc, &mut acts);
                            });
                        self.visible_rows = vis;
                    }
                }
            });

        // right panels
        egui::SidePanel::right("types")
            .frame(
                egui::Frame::default()
                    .fill(t.panel)
                    .inner_margin(egui::Margin::symmetric(10.0, 8.0)),
            )
            .default_width(340.0)
            .width_range(260.0..=480.0)
            .show(ctx, |ui| {
                let half = ui.available_height() / 2.0 - 24.0;
                ui.label(RichText::new("FILE TYPES").size(12.5).color(t.ink3));
                ui.add_space(4.0);
                egui::ScrollArea::vertical()
                    .id_salt("types_scroll")
                    .max_height(half)
                    .auto_shrink([false, false])
                    .show(ui, |ui| {
                        self.types_panel(ui, t, &mut acts);
                    });
                ui.add_space(8.0);
                ui.separator();
                ui.label(RichText::new("LARGEST FILES").size(12.5).color(t.ink3));
                ui.add_space(4.0);
                egui::ScrollArea::vertical()
                    .id_salt("largest_scroll")
                    .auto_shrink([false, false])
                    .show(ui, |ui| {
                        self.largest_panel(ui, t, &mut acts);
                    });
            });

        // center map
        egui::CentralPanel::default()
            .frame(
                egui::Frame::default()
                    .fill(t.bg)
                    .inner_margin(egui::Margin::symmetric(12.0, 8.0)),
            )
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    if self.crumbs.len() > 1 {
                        if ui
                            .button("↑ up")
                            .on_hover_text("Go up one level (Backspace)")
                            .clicked()
                        {
                            acts.push(Act::Zoom(self.crumbs[self.crumbs.len() - 2].0));
                        }
                    }
                    let has_vol = {
                        let g = core.store.read().unwrap();
                        g.as_ref().map(|s| scan::volume_of(s, self.zoom).is_some()).unwrap_or(false)
                    };
                    let mut changed = false;
                    ui.menu_button("⚙ options", |ui| {
                        ui.set_min_width(240.0);
                        ui.label(RichText::new("COLOUR BY").size(11.5).color(t.ink3));
                        for (m, lbl) in [
                            (ColorMode::Type, "File type"),
                            (ColorMode::Folder, "Top-level folder"),
                            (ColorMode::Age, "Age (last modified)"),
                        ] {
                            if ui.selectable_label(self.prefs.color_mode == m, lbl).clicked() {
                                self.prefs.color_mode = m;
                                changed = true;
                            }
                        }
                        ui.separator();
                        ui.label(RichText::new("TREEMAP").size(11.5).color(t.ink3));
                        changed |= ui.checkbox(&mut self.prefs.cushions, "Cushion shading").changed();
                        changed |= ui.checkbox(&mut self.prefs.outlines, "Folder outlines").changed();
                        changed |= ui.checkbox(&mut self.prefs.labels, "Labels").changed();
                        ui.separator();
                        ui.label(RichText::new("VOLUME BLOCKS").size(11.5).color(t.ink3));
                        ui.add_enabled_ui(has_vol, |ui| {
                            changed |= ui
                                .checkbox(&mut self.prefs.show_free, "Show free space")
                                .on_hover_text("Draw the volume's free space as a block, so the map equals the whole drive")
                                .changed();
                            changed |= ui
                                .checkbox(&mut self.prefs.show_unknown, "Show unknown")
                                .on_hover_text("Used space that is not in the tree: filesystem metadata, unreadable folders, excluded files")
                                .changed();
                        });
                        if !has_vol {
                            ui.label(RichText::new("(zoom to a volume root to show these)").size(11.5).color(t.ink3));
                        }
                    });
                    if changed {
                        self.save_prefs();
                        self.invalidate_map();
                    }
                    for (v, label) in [
                        (View::Map, "▦ map"),
                        (View::Heat, "▤ heat"),
                        (View::Rings, "○ rings"),
                    ] {
                        if ui.selectable_label(self.view == v, label).clicked()
                            && self.view != v
                        {
                            self.view = v;
                            self.invalidate_map();
                        }
                    }
                    if self.ext_sel.is_some() && ui.button("✕ clear filter").clicked() {
                        self.ext_sel = None;
                    }
                    ui.separator();
                    egui::ScrollArea::horizontal()
                        .id_salt("crumb_scroll")
                        .max_height(30.0)
                        .stick_to_right(true)
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                let crumbs = self.crumbs.clone();
                                for (i, (id, name)) in crumbs.iter().enumerate() {
                                    if i > 0 {
                                        ui.label(RichText::new("/").color(t.ink3));
                                    }
                                    let last = i == crumbs.len() - 1;
                                    let disp = truncate_to(name, 260.0, 14.0);
                                    if last {
                                        ui.label(RichText::new(disp).strong());
                                    } else if ui
                                        .link(RichText::new(disp).color(t.ink2))
                                        .clicked()
                                    {
                                        acts.push(Act::Zoom(*id));
                                    }
                                }
                            });
                        });
                });
                ui.add_space(4.0);

                let avail = ui.available_size();
                let (outer, resp) = ui.allocate_exact_size(avail, Sense::click());
                let painter = ui.painter_at(outer);
                painter.rect_filled(outer, 8.0, t.well);
                painter.rect_stroke(outer, 8.0, Stroke::new(1.0, t.line));

                {
                    let guard = core.store.read().unwrap();
                    if let Some(s) = guard.as_ref() {
                        self.relayout(s, outer.width(), outer.height());
                        if self.crumbs.last().map(|c| c.0) != Some(self.zoom) {
                            self.rebuild_crumbs(s);
                        }
                    }
                }

                let origin = outer.min;
                let hover_pos = resp.hover_pos();
                match self.view {
                    View::Map => self.paint_map(&painter, origin, t),
                    View::Heat => self.paint_heat(&painter, origin, outer, t),
                    View::Rings => self.paint_rings(&painter, outer, t),
                }

                if self.view == View::Map && self.prefs.color_mode != ColorMode::Type {
                    self.paint_legend(&painter, outer, t);
                }
                let hit: Option<(u32, String, u64, bool, u8)> = hover_pos.and_then(|p| {
                    let lx = p.x - origin.x;
                    let ly = p.y - origin.y;
                    match self.view {
                        View::Map => self
                            .rects
                            .iter()
                            .find(|r| {
                                lx >= r.x && lx < r.x + r.w && ly >= r.y && ly < r.y + r.h
                            })
                            .map(|r| (r.i, r.n.clone(), r.z, r.d != 0, r.d)),
                        View::Heat => self
                            .heat_cells
                            .iter()
                            .find(|c| c.rect.contains(Pos2::new(lx, ly)))
                            .map(|c| {
                                (c.id, c.name.clone(), c.size, c.dir, if c.dir { 1 } else { 0 })
                            }),
                        View::Rings => {
                            let cc = outer.center();
                            let dx = p.x - cc.x;
                            let dy = p.y - cc.y;
                            let r = (dx * dx + dy * dy).sqrt();
                            let mut a = dy.atan2(dx);
                            if a < 0.0 {
                                a += std::f32::consts::TAU;
                            }
                            self.sectors
                                .iter()
                                .find(|s| r >= s.r0 && r < s.r1 && a >= s.a0 && a < s.a1)
                                .map(|s| (s.id, s.name.clone(), s.size, s.d == 1, s.d))
                        }
                    }
                });

                if let Some((id, name, size, _isdir, kind)) = &hit {
                    let zsize = {
                        let guard = core.store.read().unwrap();
                        guard
                            .as_ref()
                            .map(|s| s.nodes[self.zoom as usize].size)
                            .unwrap_or(1)
                    };
                    let extra = match kind {
                        1 => "folder · double-click to zoom",
                        2 => "many small items · double-click to zoom",
                        3 => "free space on this volume",
                        4 => "used on disk but not in this tree: filesystem metadata, unreadable or excluded folders",
                        _ => "",
                    };
                    egui::show_tooltip_at_pointer(
                        ui.ctx(),
                        ui.layer_id(),
                        egui::Id::new("map_tip"),
                        |ui| {
                            ui.set_min_width(250.0);
                            ui.set_max_width(420.0);
                            ui.label(
                                RichText::new(truncate_to(name, 400.0, 15.5))
                                    .strong()
                                    .size(15.5),
                            );
                            ui.label(
                                RichText::new(format!(
                                    "{} · {} of this view",
                                    fmt_bytes(*size),
                                    fmt_pct(100.0 * *size as f64 / zsize.max(1) as f64)
                                ))
                                .size(14.0),
                            );
                            if !extra.is_empty() {
                                ui.label(RichText::new(extra).size(12.5).color(t.ink3));
                            }
                        },
                    );

                    let ctrl = ui.input(|i| i.modifiers.command);
                    if resp.clicked() && *kind < 3 {
                        acts.push(if ctrl { Act::ToggleSelect(*id) } else { Act::Select(*id) });
                    }
                    if resp.double_clicked() {
                        match kind {
                            1 | 2 => acts.push(Act::Zoom(*id)),
                            3 | 4 => {}
                            _ => {
                                let guard = core.store.read().unwrap();
                                if let Some(s) = guard.as_ref() {
                                    if let Some(p) = scan::ancestors_of(s, *id).last() {
                                        acts.push(Act::Zoom(*p));
                                    }
                                }
                            }
                        }
                    }
                }
                // pin the target at the moment of right-click so the menu
                // stays stable while the pointer travels over it
                if resp.secondary_clicked() {
                    self.ctx_target = hit.clone();
                }
                let menu_target = self.ctx_target.clone();
                let multi_n = if self.sel_multi.len() > 1 { self.sel_multi.len() } else { 0 };
                let multi_has = |id: u32| multi_n > 0 && self.sel_multi.contains(&id);
                resp.context_menu(|ui| {
                    if let Some((id, name, size, isdir, kind)) = &menu_target {
                        if *kind >= 3 {
                            ui.close_menu();
                        } else {
                            node_menu(ui, *id, name, *size, *isdir, if multi_has(*id) { multi_n } else { 0 }, &mut acts);
                        }
                    } else {
                        ui.close_menu();
                    }
                });
            });

        self.delete_modal(ctx, t);
        self.props_window(ctx, t);
        self.trash_window(ctx, t);
        self.about_window(ctx, t);
        self.paint_toast(ctx, t);

        for a in acts {
            self.apply(a);
        }
        if theme_flip {
            self.theme_kind = match self.theme_kind {
                ThemeKind::Dark => ThemeKind::Light,
                ThemeKind::Light => ThemeKind::Dark,
            };
            apply_style(ctx, self.theme_kind);
            self.invalidate_map(); // cached mesh colors are theme-baked
            let _ = std::fs::create_dir_all(dirs_config());
            let _ = std::fs::write(
                &self.prefs_path,
                if self.theme_kind == ThemeKind::Light { "light" } else { "dark" },
            );
        }
        if rescan {
            self.rescan_last();
        }
        if retarget {
            self.show_landing = true;
            self.roots = list_roots();
        }
    }

    fn types_panel(&mut self, ui: &mut egui::Ui, t: &Theme, acts: &mut Vec<Act>) {
        let Some(s) = &self.snap else { return };
        let total = s.bytes.max(1);
        // sortable header
        let (key, desc) = self.types_sort;
        let mut click: Option<TypesSort> = None;
        ui.horizontal(|ui| {
            let w = ui.available_width();
            for (k, lbl, width, right) in [
                (TypesSort::Name, "TYPE", (w - 180.0).max(60.0), false),
                (TypesSort::Bytes, "SIZE", 90.0, true),
                (TypesSort::Files, "FILES", 80.0, true),
            ] {
                let on = key == k;
                let arrow = if on { if desc { " ▾" } else { " ▴" } } else { "" };
                let (rect, resp) = ui.allocate_exact_size(Vec2::new(width, 18.0), Sense::click());
                let pos = if right { Pos2::new(rect.max.x - 4.0, rect.center().y) } else { Pos2::new(rect.min.x + 4.0, rect.center().y) };
                ui.painter().text(
                    pos,
                    if right { Align2::RIGHT_CENTER } else { Align2::LEFT_CENTER },
                    format!("{}{}", lbl, arrow),
                    FontId::new(11.5, FontFamily::Monospace),
                    if on { t.acc } else { t.ink3 },
                );
                if resp.clicked() {
                    click = Some(k);
                }
            }
        });
        if let Some(k) = click {
            self.types_sort = if key == k { (k, !desc) } else { (k, k != TypesSort::Name) };
        }
        let (key, desc) = self.types_sort;
        let mut order: Vec<usize> = (0..s.exts.len()).collect();
        match key {
            TypesSort::Bytes => order.sort_by(|a, b| s.exts[*b].bytes.cmp(&s.exts[*a].bytes)),
            TypesSort::Files => order.sort_by(|a, b| s.exts[*b].files.cmp(&s.exts[*a].files)),
            TypesSort::Name => order.sort_by(|a, b| s.exts[*a].name.cmp(&s.exts[*b].name)),
        }
        if !desc {
            order.reverse();
        }
        for i in order {
            let e = &s.exts[i];
            let c = t.slot_color(e.slot);
            let selectable = e.slot < 8;
            let active = self.ext_sel == Some(e.slot);
            let label = if e.name.is_empty() {
                "no extension".to_string()
            } else {
                format!(".{}", e.name)
            };
            let frac = e.bytes as f32 / total as f32;
            let resp = ui.allocate_response(
                Vec2::new(ui.available_width(), 48.0),
                if selectable { Sense::click() } else { Sense::hover() },
            );
            let r = resp.rect;
            if active {
                ui.painter().rect_filled(r, 5.0, t.acc.gamma_multiply(0.12));
            } else if resp.hovered() && selectable {
                ui.painter().rect_filled(r, 5.0, t.ink.gamma_multiply(0.05));
            }
            let p = ui.painter();
            p.rect_filled(
                Rect::from_min_size(r.min + Vec2::new(6.0, 9.0), Vec2::splat(12.0)),
                3.0,
                c,
            );
            let name_w = (r.width() - 150.0).max(60.0);
            let shown = truncate_to(&label, name_w, 15.5);
            p.text(
                r.min + Vec2::new(26.0, 4.0),
                Align2::LEFT_TOP,
                &shown,
                FontId::new(15.5, FontFamily::Monospace),
                t.ink,
            );
            if !e.desc.is_empty() && !e.name.is_empty() {
                let used = shown.chars().count() as f32 * 15.5 * 0.62 + 10.0;
                let room = name_w - used;
                if room > 40.0 {
                    p.text(
                        r.min + Vec2::new(26.0 + used, 7.0),
                        Align2::LEFT_TOP,
                        truncate_to(e.desc, room, 12.0),
                        FontId::new(12.0, FontFamily::Monospace),
                        t.ink3,
                    );
                }
            }
            p.text(
                Pos2::new(r.max.x - 8.0, r.min.y + 4.0),
                Align2::RIGHT_TOP,
                fmt_bytes(e.bytes),
                FontId::new(14.5, FontFamily::Monospace),
                t.ink2,
            );
            let bar = Rect::from_min_size(
                r.min + Vec2::new(26.0, 30.0),
                Vec2::new((r.width() - 190.0).max(30.0), 6.0),
            );
            p.rect_filled(bar, 3.0, t.line);
            let mut fb = bar;
            fb.set_right(bar.left() + bar.width() * frac.max(0.005));
            p.rect_filled(fb, 3.0, c);
            p.text(
                Pos2::new(r.max.x - 8.0, r.min.y + 27.0),
                Align2::RIGHT_TOP,
                format!("{} · {} files", fmt_pct(100.0 * frac as f64), fmt_n(e.files)),
                FontId::new(12.5, FontFamily::Monospace),
                t.ink3,
            );
            let resp = if e.desc.is_empty() { resp } else { resp.on_hover_text(e.desc) };
            if selectable && resp.clicked() {
                acts.push(Act::ToggleExt(e.slot));
            }
        }
    }

    fn largest_panel(&self, ui: &mut egui::Ui, t: &Theme, acts: &mut Vec<Act>) {
        let Some(s) = &self.snap else { return };
        for (i, b) in s.largest.iter().enumerate() {
            let resp =
                ui.allocate_response(Vec2::new(ui.available_width(), 30.0), Sense::click());
            let r = resp.rect;
            if resp.hovered() {
                ui.painter().rect_filled(r, 5.0, t.ink.gamma_multiply(0.05));
            }
            let p = ui.painter();
            p.text(
                r.min + Vec2::new(4.0, 6.0),
                Align2::LEFT_TOP,
                format!("{:>2}", i + 1),
                FontId::new(13.0, FontFamily::Monospace),
                t.ink3,
            );
            p.rect_filled(
                Rect::from_min_size(r.min + Vec2::new(30.0, 10.0), Vec2::splat(10.0)),
                2.0,
                t.slot_color(b.slot),
            );
            let shown = truncate_to(&b.name, (r.width() - 150.0).max(40.0), 15.0);
            let clipped = shown.ends_with('…');
            p.text(
                r.min + Vec2::new(48.0, 6.0),
                Align2::LEFT_TOP,
                shown,
                FontId::new(15.0, FontFamily::Monospace),
                t.ink,
            );
            p.text(
                Pos2::new(r.max.x - 6.0, r.min.y + 6.0),
                Align2::RIGHT_TOP,
                fmt_bytes(b.size),
                FontId::new(14.5, FontFamily::Monospace),
                t.ink2,
            );
            if resp.clicked() {
                acts.push(Act::Select(b.id));
            }
            let resp = if clipped {
                resp.on_hover_text(&b.name)
            } else {
                resp
            };
            resp.context_menu(|ui| {
                node_menu(ui, b.id, &b.name, b.size, false, 0, acts);
            });
        }
    }

    fn paint_map(&self, p: &egui::Painter, o: Pos2, t: &Theme) {
        let mut mesh = self.cached_mesh.clone();
        mesh.translate(o.to_vec2());
        p.add(egui::Shape::mesh(mesh));

        if let Some(slot) = self.ext_sel {
            let dim = if self.theme_kind == ThemeKind::Dark {
                Color32::from_black_alpha(190)
            } else {
                Color32::from_white_alpha(200)
            };
            for r in &self.rects {
                if r.s != slot {
                    p.rect_filled(
                        Rect::from_min_size(
                            Pos2::new(o.x + r.x, o.y + r.y),
                            Vec2::new(r.w, r.h),
                        ),
                        0.0,
                        dim,
                    );
                }
            }
        }

        if self.prefs.outlines {
            for d in &self.dir_rects {
                if d.p > 5 {
                    continue;
                }
                let a = (0.42 - d.p as f32 * 0.08).max(0.08);
                p.rect_stroke(
                    Rect::from_min_size(Pos2::new(o.x + d.x, o.y + d.y), Vec2::new(d.w, d.h)),
                    0.0,
                    Stroke::new(1.0, Color32::from_black_alpha((a * 255.0) as u8)),
                );
            }
        }
        // pseudo blocks get a dashed frame so they read as "not files"
        for r in &self.rects {
            if r.d >= 3 {
                let rr = Rect::from_min_size(Pos2::new(o.x + r.x, o.y + r.y), Vec2::new(r.w, r.h));
                p.rect_stroke(rr.shrink(1.0), 0.0, Stroke::new(1.0, t.ink3.gamma_multiply(0.7)));
            }
        }

        let label_col = if self.theme_kind == ThemeKind::Dark {
            Color32::from_rgba_unmultiplied(238, 242, 248, 220)
        } else {
            Color32::from_rgba_unmultiplied(20, 26, 36, 235)
        };
        let dir_col = label_col.gamma_multiply(0.92);
        let font = FontId::new(13.0, FontFamily::Monospace);
        for (pos, text, is_dir) in &self.cached_labels {
            p.text(
                Pos2::new(o.x + pos.x, o.y + pos.y),
                Align2::LEFT_TOP,
                text,
                font.clone(),
                if *is_dir { dir_col } else { label_col },
            );
        }
        let sels: Vec<u32> = if self.sel_multi.len() > 1 { self.sel_multi.iter().copied().collect() } else { self.sel.into_iter().collect() };
        for sel in sels {
            let target = self
                .rects
                .iter()
                .find(|r| r.i == sel && r.d == 0)
                .or_else(|| self.rects.iter().find(|r| r.i == sel && r.d < 3))
                .map(|r| Rect::from_min_size(Pos2::new(o.x + r.x, o.y + r.y), Vec2::new(r.w, r.h)))
                .or_else(|| {
                    self.dir_rects.iter().find(|d| d.i == sel).map(|d| {
                        Rect::from_min_size(Pos2::new(o.x + d.x, o.y + d.y), Vec2::new(d.w, d.h))
                    })
                });
            if let Some(r) = target {
                p.rect_stroke(r, 0.0, Stroke::new(2.0, t.ink));
            }
        }
    }

    fn paint_heat(&self, p: &egui::Painter, o: Pos2, outer: Rect, t: &Theme) {
        for c in &self.heat_cells {
            let r = c.rect.translate(o.to_vec2());
            let dimmed = self.ext_sel.map(|s| c.slot != s).unwrap_or(false);
            let col = if dimmed {
                t.heat[c.lvl as usize].gamma_multiply(0.25)
            } else {
                t.heat[c.lvl as usize]
            };
            let rounding = Rounding::same((r.width() * 0.22).min(5.0));
            p.rect_filled(r, rounding, col);
            if c.dir {
                p.rect_stroke(r, rounding, Stroke::new(1.0, t.ink.gamma_multiply(0.45)));
            }
            if self.sel == Some(c.id) {
                p.rect_stroke(r.expand(1.5), Rounding::same(5.0), Stroke::new(2.0, t.ink));
            }
        }
        let ly = outer.max.y - 26.0;
        let lx = outer.max.x - 190.0;
        let font = FontId::new(13.0, FontFamily::Monospace);
        p.text(
            Pos2::new(lx - 8.0, ly + 7.0),
            Align2::RIGHT_CENTER,
            "less",
            font.clone(),
            t.ink3,
        );
        for (i, c) in t.heat.iter().enumerate() {
            p.rect_filled(
                Rect::from_min_size(Pos2::new(lx + i as f32 * 19.0, ly), Vec2::splat(14.0)),
                3.0,
                *c,
            );
        }
        p.text(
            Pos2::new(lx + 5.0 * 19.0 + 6.0, ly + 7.0),
            Align2::LEFT_CENTER,
            "more",
            font.clone(),
            t.ink3,
        );
        let cap = format!(
            "{} items · one cell each · ring = folder · color = size (log){}",
            fmt_n(self.heat_cells.len() as u64),
            if self.heat_more > 0 {
                format!(" · {} smaller not shown", fmt_n(self.heat_more as u64))
            } else {
                String::new()
            }
        );
        let cap_w = (outer.width() - 280.0).max(80.0);
        p.text(
            Pos2::new(outer.min.x + 18.0, ly + 7.0),
            Align2::LEFT_CENTER,
            truncate_to(&cap, cap_w, 13.0),
            font,
            t.ink3,
        );
    }

    fn paint_rings(&self, p: &egui::Painter, outer: Rect, t: &Theme) {
        let c = outer.center();
        let mut mesh = egui::Mesh::default();
        for s in &self.sectors {
            let dimmed = self.ext_sel.map(|x| s.slot != x).unwrap_or(false);
            let mut col = t.slot_color(s.slot);
            if dimmed {
                col = col.gamma_multiply(0.22);
            }
            let steps = (((s.a1 - s.a0) / 0.05).ceil() as usize).clamp(2, 220);
            let gap = 0.0015_f32.min((s.a1 - s.a0) * 0.08);
            let (a0, a1) = (s.a0 + gap, s.a1 - gap);
            let inner = lighten(col, 0.12);
            let outer_c = darken(col, 0.14);
            let mut prev: Option<(u32, u32)> = None;
            for i in 0..=steps {
                let a = a0 + (a1 - a0) * i as f32 / steps as f32;
                let (sin, cos) = a.sin_cos();
                let vi = mesh.vertices.len() as u32;
                mesh.colored_vertex(Pos2::new(c.x + cos * s.r0, c.y + sin * s.r0), inner);
                mesh.colored_vertex(Pos2::new(c.x + cos * s.r1, c.y + sin * s.r1), outer_c);
                if let Some((v0, v1)) = prev {
                    mesh.add_triangle(v0, v1, vi);
                    mesh.add_triangle(v1, vi + 1, vi);
                }
                prev = Some((vi, vi + 1));
            }
        }
        p.add(egui::Shape::mesh(mesh));

        // selection outline traces the sector
        if let Some(sel) = self.sel {
            if let Some(s) = self.sectors.iter().find(|s| s.id == sel && s.d != 2) {
                let steps = (((s.a1 - s.a0) / 0.05).ceil() as usize).clamp(2, 220);
                let mut pts = Vec::with_capacity(steps * 2 + 2);
                for i in 0..=steps {
                    let a = s.a0 + (s.a1 - s.a0) * i as f32 / steps as f32;
                    let (sin, cos) = a.sin_cos();
                    pts.push(Pos2::new(c.x + cos * s.r1, c.y + sin * s.r1));
                }
                for i in (0..=steps).rev() {
                    let a = s.a0 + (s.a1 - s.a0) * i as f32 / steps as f32;
                    let (sin, cos) = a.sin_cos();
                    pts.push(Pos2::new(c.x + cos * s.r0, c.y + sin * s.r0));
                }
                p.add(egui::Shape::closed_line(pts, Stroke::new(2.0, t.ink)));
            }
        }

        // center label
        p.text(
            c - Vec2::new(0.0, 12.0),
            Align2::CENTER_CENTER,
            truncate_to(&self.ring_label.0, 170.0, 15.0),
            FontId::new(15.0, FontFamily::Monospace),
            t.ink,
        );
        p.text(
            c + Vec2::new(0.0, 12.0),
            Align2::CENTER_CENTER,
            &self.ring_label.1,
            FontId::new(14.0, FontFamily::Monospace),
            t.ink3,
        );
    }

    fn dim(ctx: &egui::Context, id: &str) {
        let screen = ctx.screen_rect();
        egui::Area::new(egui::Id::new(id))
            .fixed_pos(screen.min)
            .order(egui::Order::Foreground)
            .show(ctx, |ui| {
                // swallow every click outside the dialog: the modal is modal
                let _ = ui.allocate_response(screen.size(), egui::Sense::click());
                ui.painter().rect_filled(screen, 0.0, Color32::from_black_alpha(160));
            });
    }

    fn delete_modal(&mut self, ctx: &egui::Context, t: &Theme) {
        let Some(targets) = self.delete_target.clone() else {
            return;
        };
        Self::dim(ctx, "modal_dim");
        let total: u64 = targets.iter().map(|d| d.size).sum();
        let files: u64 = targets.iter().map(|d| if d.dir { d.files as u64 } else { 1 }).sum();
        let title = if targets.len() == 1 {
            "Delete this item?".to_string()
        } else {
            format!("Delete {} items?", targets.len())
        };
        let mut close = false;
        let mut confirm: Option<bool> = None; // Some(to_trash)
        egui::Window::new(RichText::new(title).size(19.0).strong())
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .show(ctx, |ui| {
                ui.set_width(500.0);
                egui::Frame::default()
                    .fill(t.well)
                    .stroke(Stroke::new(1.0, t.line))
                    .inner_margin(12.0)
                    .rounding(6.0)
                    .show(ui, |ui| {
                        if targets.len() == 1 {
                            let d = &targets[0];
                            ui.label(RichText::new(&d.name).strong().size(16.0));
                            ui.label(RichText::new(&d.path).size(13.0).color(t.ink3));
                        } else {
                            for d in targets.iter().take(8) {
                                ui.label(
                                    RichText::new(format!("{}  ·  {}", truncate_to(&d.path, 380.0, 13.0), fmt_bytes(d.size)))
                                        .size(13.0)
                                        .color(t.ink2),
                                );
                            }
                            if targets.len() > 8 {
                                ui.label(RichText::new(format!("… and {} more", targets.len() - 8)).size(13.0).color(t.ink3));
                            }
                        }
                        ui.label(
                            RichText::new(format!("{} · {} files", fmt_bytes(total), fmt_n(files)))
                                .size(14.0)
                                .color(t.ink2),
                        );
                    });
                ui.add_space(8.0);
                ui.label(
                    RichText::new(
                        "Move to trash keeps a copy in your system's trash / recycle bin so you can \
                         restore it. Delete permanently frees the space now — there is no undo.",
                    )
                    .size(13.0)
                    .color(t.ink2),
                );
                ui.add_space(10.0);
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let dark_ink = if self.theme_kind == ThemeKind::Dark { hex("#161006") } else { hex("#FFFFFF") };
                    let trash_btn = egui::Button::new(
                        RichText::new("Move to trash").strong().color(dark_ink).size(15.0),
                    )
                    .fill(t.acc);
                    if ui.add(trash_btn).on_hover_text("Enter").clicked() {
                        confirm = Some(true);
                    }
                    let del = egui::Button::new(
                        RichText::new("Delete permanently").color(t.danger).size(15.0),
                    )
                    .stroke(Stroke::new(1.0, t.danger));
                    if ui.add(del).clicked() {
                        confirm = Some(false);
                    }
                    if ui.button("Cancel").clicked() {
                        close = true;
                    }
                });
            });
        if ctx.input(|i| i.key_pressed(egui::Key::Enter)) {
            confirm = Some(true);
        }
        if close {
            self.delete_target = None;
        }
        if let Some(to_trash) = confirm {
            self.delete_target = None;
            self.do_delete(targets, to_trash);
        }
    }

    fn props_window(&mut self, ctx: &egui::Context, t: &Theme) {
        let Some(p) = self.props.clone() else { return };
        Self::dim(ctx, "props_dim");
        let mut close = false;
        egui::Window::new(RichText::new("Properties").size(19.0).strong())
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .show(ctx, |ui| {
                ui.set_width(520.0);
                ui.label(RichText::new(&p.name).strong().size(17.0));
                ui.add_space(6.0);
                egui::Grid::new("props_grid").num_columns(2).spacing([18.0, 7.0]).show(ui, |ui| {
                    let row = |ui: &mut egui::Ui, k: &str, v: String| {
                        ui.label(RichText::new(k).size(12.5).color(t.ink3));
                        ui.add(egui::Label::new(RichText::new(v).size(14.0)).wrap());
                        ui.end_row();
                    };
                    row(ui, "LOCATION", p.path.clone());
                    row(
                        ui,
                        "KIND",
                        if p.dir {
                            "folder".into()
                        } else if p.ext.is_empty() {
                            "file (no extension)".into()
                        } else {
                            let d = scan::ext_description(&p.ext);
                            if d.is_empty() { format!(".{} file", p.ext) } else { format!(".{} file — {}", p.ext, d) }
                        },
                    );
                    row(ui, "SIZE ON DISK", format!("{}  ({} bytes)", fmt_bytes(p.size), fmt_n(p.size)));
                    if p.dir {
                        row(ui, "CONTAINS", format!("{} files · {} folders", fmt_n(p.files as u64), fmt_n(p.subdirs as u64)));
                    }
                    row(ui, "LAST CHANGE", if p.mtime > 0 { scan::fmt_date(p.mtime) } else { "unknown".into() });
                    row(ui, "OF PARENT", fmt_pct(p.pct_parent));
                    row(ui, "OF SCAN", fmt_pct(p.pct_total));
                });
                ui.add_space(10.0);
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.button("Close").clicked() {
                        close = true;
                    }
                    if ui.button("❐  Copy path").clicked() {
                        self.pending_copy = Some(p.path.clone());
                    }
                });
            });
        if close {
            self.props = None;
        }
    }

    fn trash_window(&mut self, ctx: &egui::Context, t: &Theme) {
        // result of an emptying in flight
        if let Some(slot) = &self.trash_busy {
            ctx.request_repaint_after(std::time::Duration::from_millis(200));
            let done = slot.lock().ok().and_then(|mut g| g.take());
            if let Some(r) = done {
                self.trash_busy = None;
                match r {
                    Ok(n) => self.toast(format!("Trash emptied — {} items gone", fmt_n(n)), false),
                    Err(e) => self.toast(format!("could not empty trash: {}", e), true),
                }
            }
        }
        let Some(count) = self.trash_confirm else { return };
        Self::dim(ctx, "trash_dim");
        let mut close = false;
        let mut go = false;
        egui::Window::new(RichText::new("Empty the trash?").size(19.0).strong())
            .collapsible(false)
            .resizable(false)
            .order(egui::Order::Foreground)
            .anchor(Align2::CENTER_CENTER, Vec2::ZERO)
            .show(ctx, |ui| {
                ui.set_width(440.0);
                ui.label(
                    RichText::new(format!(
                        "{} item{} in your system trash / recycle bin will be removed for good.",
                        fmt_n(count),
                        if count == 1 { "" } else { "s" }
                    ))
                    .size(14.0)
                    .color(t.ink2),
                );
                ui.add_space(10.0);
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let del = egui::Button::new(RichText::new("Empty trash").color(t.danger).size(15.0))
                        .stroke(Stroke::new(1.0, t.danger));
                    if ui.add_enabled(count > 0, del).clicked() {
                        go = true;
                    }
                    if ui.button("Cancel").clicked() {
                        close = true;
                    }
                });
            });
        if close || go {
            self.trash_confirm = None;
        }
        if go {
            let slot: Arc<std::sync::Mutex<Option<Result<u64, String>>>> = Arc::new(std::sync::Mutex::new(None));
            let w = slot.clone();
            std::thread::spawn(move || {
                let r = crate::empty_trash();
                if let Ok(mut g) = w.lock() {
                    *g = Some(r);
                }
            });
            self.trash_busy = Some(slot);
        }
    }

    /// Small colour key in the map's bottom-left for folder / age modes.
    fn paint_legend(&self, p: &egui::Painter, outer: Rect, t: &Theme) {
        let entries: Vec<(Color32, String)> = match self.prefs.color_mode {
            ColorMode::Age => AGE_BUCKETS
                .iter()
                .enumerate()
                .map(|(i, b)| (age_color(t, i), b.0.to_string()))
                .collect(),
            ColorMode::Folder => {
                let core = self.core.clone();
                let g = core.store.read().unwrap();
                let Some(s) = g.as_ref() else { return };
                sorted_children(s, self.zoom, SortKey::Size, true)
                    .into_iter()
                    .take(8)
                    .enumerate()
                    .map(|(i, c)| (t.slots[i], truncate_to(&s.nodes[c as usize].name, 120.0, 12.0)))
                    .collect()
            }
            ColorMode::Type => return,
        };
        let font = FontId::new(12.0, FontFamily::Monospace);
        let mut x = outer.min.x + 10.0;
        let y = outer.max.y - 22.0;
        let mut w = 0.0;
        for (_, l) in &entries {
            w += 22.0 + l.chars().count() as f32 * 7.5 + 10.0;
        }
        let bg = Rect::from_min_size(Pos2::new(x - 6.0, y - 6.0), Vec2::new(w + 8.0, 24.0));
        p.rect_filled(bg, 5.0, t.panel.gamma_multiply(0.9));
        for (c, l) in &entries {
            p.rect_filled(Rect::from_min_size(Pos2::new(x, y), Vec2::splat(12.0)), 2.0, *c);
            p.text(Pos2::new(x + 17.0, y + 6.0), Align2::LEFT_CENTER, l, font.clone(), t.ink2);
            x += 22.0 + l.chars().count() as f32 * 7.5 + 10.0;
        }
    }

    fn paint_toast(&mut self, ctx: &egui::Context, t: &Theme) {
        if let Some(p) = self.pending_copy.take() {
            ctx.copy_text(p);
            self.toast("Path copied", false);
        }
        let Some((msg, at, bad)) = self.toast.clone() else { return };
        if at.elapsed().as_secs_f32() > 3.2 {
            self.toast = None;
            return;
        }
        ctx.request_repaint_after(std::time::Duration::from_millis(300));
        egui::Area::new(egui::Id::new("toast"))
            .anchor(Align2::CENTER_BOTTOM, Vec2::new(0.0, -44.0))
            .order(egui::Order::Foreground)
            .show(ctx, |ui| {
                egui::Frame::default()
                    .fill(t.panel2)
                    .stroke(Stroke::new(1.0, if bad { t.danger } else { t.acc }))
                    .rounding(7.0)
                    .inner_margin(egui::Margin::symmetric(14.0, 10.0))
                    .show(ui, |ui| {
                        ui.label(RichText::new(msg).size(15.0));
                    });
            });
    }
}

fn age_color(t: &Theme, bucket: usize) -> Color32 {
    // newest = hottest; the sixth bucket reuses the coolest heat tone dimmed
    match bucket {
        0 => t.heat[4],
        1 => t.heat[3],
        2 => t.heat[2],
        3 => t.heat[1],
        4 => lighten(t.heat[0], 0.18),
        _ => t.heat[0],
    }
}

#[allow(clippy::too_many_arguments)]
fn node_menu(ui: &mut egui::Ui, id: u32, name: &str, size: u64, is_dir: bool, multi: usize, acts: &mut Vec<Act>) {
    ui.set_min_width(300.0);
    ui.label(RichText::new(truncate_to(name, 380.0, 15.0)).strong());
    ui.label(RichText::new(fmt_bytes(size)).small());
    ui.separator();
    if ui.button("↗  Open").clicked() {
        acts.push(Act::Open(id));
        ui.close_menu();
    }
    if ui.button("▸  Reveal in file manager").clicked() {
        acts.push(Act::Reveal(id));
        ui.close_menu();
    }
    if ui.button("▸  Open terminal here").clicked() {
        acts.push(Act::Terminal(id));
        ui.close_menu();
    }
    if ui.button("❐  Copy path").clicked() {
        acts.push(Act::CopyPath(id));
        ui.close_menu();
    }
    if ui.button("☰  Properties…").clicked() {
        acts.push(Act::Props(id));
        ui.close_menu();
    }
    ui.separator();
    if is_dir {
        if ui.button("◎  Zoom here").clicked() {
            acts.push(Act::Zoom(id));
            ui.close_menu();
        }
        if ui.button("⟳  Rescan this folder").clicked() {
            acts.push(Act::RescanFolder(id));
            ui.close_menu();
        }
        if ui.button("⇩  Export CSV of this folder").clicked() {
            acts.push(Act::ExportCsv(id));
            ui.close_menu();
        }
    } else if ui.button("◎  Zoom to parent folder").clicked() {
        acts.push(Act::ZoomParent(id));
        ui.close_menu();
    }
    if ui.button("◉  Locate in explorer").clicked() {
        acts.push(Act::Locate(id));
        ui.close_menu();
    }
    ui.separator();
    if multi > 1 {
        if ui
            .button(RichText::new(format!("✕  Delete {} selected items…", multi)).color(hex("#E66767")))
            .clicked()
        {
            acts.push(Act::AskDeleteSelected);
            ui.close_menu();
        }
    } else if id == 0 {
        ui.add_enabled(false, egui::Button::new("✕  Delete…"));
    } else if ui
        .button(RichText::new("✕  Delete… (trash or permanent)").color(hex("#E66767")))
        .clicked()
    {
        acts.push(Act::AskDelete(id));
        ui.close_menu();
    }
}

#[derive(Clone, Copy)]
struct TreeCols {
    items: bool,
    date: bool,
}

struct TreeCtx<'a> {
    sort: (SortKey, bool),
    sel: Option<u32>,
    sel_set: &'a BTreeSet<u32>,
    scroll_to: Option<u32>,
    cols: TreeCols,
    /// every row drawn this frame, top to bottom (keyboard navigation)
    vis: &'a mut Vec<u32>,
}

fn draw_tree(
    ui: &mut egui::Ui,
    s: &Store,
    id: u32,
    depth: usize,
    t: &Theme,
    tc: &mut TreeCtx,
    acts: &mut Vec<Act>,
) {
    if depth > 32 {
        return;
    }
    let n = &s.nodes[id as usize];
    const CAP: usize = 400;
    let mut kids = sorted_children(s, id, tc.sort.0, tc.sort.1);
    let more = kids.len().saturating_sub(CAP);
    kids.truncate(CAP);
    let parent_size = n.size.max(1);

    for c in kids {
        let k = &s.nodes[c as usize];
        let pct = 100.0 * k.size as f64 / parent_size as f64;
        let selected = tc.sel == Some(c) || tc.sel_set.contains(&c);
        let items = k.files as u64 + k.subdirs as u64;
        tc.vis.push(c);
        if k.is_dir {
            let cid = egui::Id::new((s.generation, c, "tree"));
            egui::collapsing_header::CollapsingState::load_with_default_open(ui.ctx(), cid, false)
                .show_header(ui, |ui| {
                    let r = tree_row(
                        ui, t, k.name.as_ref(), pct, k.size, k.mtime, items, None, selected, acts, c, true,
                        tc.cols, false,
                    );
                    if tc.scroll_to == Some(c) {
                        ui.scroll_to_rect(r, Some(egui::Align::Center));
                    }
                })
                .body(|ui| {
                    draw_tree(ui, s, c, depth + 1, t, tc, acts);
                });
        } else {
            ui.horizontal(|ui| {
                ui.add_space(20.0);
                let slot = s.exts[k.ext as usize].slot;
                let r = tree_row(
                    ui,
                    t,
                    k.name.as_ref(),
                    pct,
                    k.size,
                    k.mtime,
                    items,
                    Some(t.slot_color(slot)),
                    selected,
                    acts,
                    c,
                    false,
                    tc.cols,
                    false,
                );
                if tc.scroll_to == Some(c) {
                    ui.scroll_to_rect(r, Some(egui::Align::Center));
                }
            });
        }
    }
    if more > 0 {
        ui.label(
            RichText::new(format!("… {} smaller items not shown", fmt_n(more as u64)))
                .size(13.0)
                .color(t.ink3),
        );
    }
}

/// One explorer row: name | items | modified | % | size. `flat` rows (search
/// results) show a full path and locate on double-click instead of zooming.
#[allow(clippy::too_many_arguments)]
fn tree_row(
    ui: &mut egui::Ui,
    t: &Theme,
    name: &str,
    pct: f64,
    size: u64,
    mtime: i64,
    items: u64,
    chip: Option<Color32>,
    selected: bool,
    acts: &mut Vec<Act>,
    id: u32,
    is_dir: bool,
    cols: TreeCols,
    flat: bool,
) -> Rect {
    let resp = ui.allocate_response(Vec2::new(ui.available_width(), 26.0), Sense::click());
    let r = resp.rect;
    if selected {
        ui.painter().rect_filled(r, 4.0, t.acc.gamma_multiply(0.13));
    } else if resp.hovered() {
        ui.painter().rect_filled(r, 4.0, t.ink.gamma_multiply(0.05));
    }
    // percent-of-parent as a colored fill: files wear their type color,
    // folders wear lapis — the row itself becomes the bar chart
    let mut fill = r;
    fill.set_right(r.left() + r.width() * (pct as f32 / 100.0).clamp(0.0, 1.0));
    let fill_col = chip.unwrap_or(t.slots[0]).gamma_multiply(0.20);
    ui.painter().rect_filled(fill, 4.0, fill_col);

    let p = ui.painter();
    let x = r.min.x + 4.0;
    if let Some(c) = chip {
        p.rect_filled(
            Rect::from_min_size(Pos2::new(x, r.center().y - 5.0), Vec2::splat(10.0)),
            2.0,
            c,
        );
    } else {
        p.rect_stroke(
            Rect::from_min_size(Pos2::new(x, r.center().y - 5.0), Vec2::splat(10.0)),
            2.0,
            Stroke::new(1.5, t.ink3),
        );
    }
    // right-aligned columns, laid out from the right edge inwards
    let mut rx = r.max.x - 6.0;
    p.text(Pos2::new(rx, r.center().y), Align2::RIGHT_CENTER, fmt_bytes(size), FontId::new(14.0, FontFamily::Monospace), t.ink2);
    rx -= 80.0;
    p.text(Pos2::new(rx, r.center().y), Align2::RIGHT_CENTER, fmt_pct(pct), FontId::new(13.0, FontFamily::Monospace), t.ink3);
    rx -= 50.0;
    if cols.date {
        p.text(Pos2::new(rx, r.center().y), Align2::RIGHT_CENTER, scan::fmt_date(mtime), FontId::new(12.5, FontFamily::Monospace), t.ink3);
        rx -= 92.0;
    }
    if cols.items && is_dir {
        p.text(Pos2::new(rx, r.center().y), Align2::RIGHT_CENTER, fmt_n(items), FontId::new(12.5, FontFamily::Monospace), t.ink3);
    }
    if cols.items {
        rx -= 64.0;
    }
    let name_w = (rx - (x + 18.0) - 6.0).max(40.0);
    let shown = truncate_to(name, name_w, 15.0);
    let clipped = shown.ends_with('…');
    p.text(
        Pos2::new(x + 18.0, r.center().y),
        Align2::LEFT_CENTER,
        shown,
        FontId::new(15.0, FontFamily::Monospace),
        t.ink,
    );

    let ctrl = ui.input(|i| i.modifiers.command);
    if resp.clicked() {
        acts.push(if ctrl { Act::ToggleSelect(id) } else { Act::Select(id) });
    }
    if resp.double_clicked() {
        if flat {
            acts.push(Act::Locate(id));
        } else if is_dir {
            acts.push(Act::Zoom(id));
        }
    }
    let hover = if clipped || flat {
        format!("{}\n{} · modified {}{}", name, fmt_bytes(size), scan::fmt_date(mtime), if is_dir { format!(" · {} items", fmt_n(items)) } else { String::new() })
    } else {
        String::new()
    };
    let resp = if hover.is_empty() { resp } else { resp.on_hover_text(hover) };
    resp.context_menu(|ui| {
        node_menu(ui, id, name, size, is_dir, 0, acts);
    });
    r
}

fn truncate_to(s: &str, max_px: f32, char_px: f32) -> String {
    let max_chars = (max_px / (char_px * 0.62)).max(4.0) as usize;
    if s.chars().count() <= max_chars {
        s.to_string()
    } else {
        let cut: String = s.chars().take(max_chars.saturating_sub(1)).collect();
        format!("{}…", cut)
    }
}

// ---------- entry ----------

pub fn run(core: Arc<Core>, initial: Option<ScanTarget>, takeover: Option<u32>) {
    match initial {
        Some(ScanTarget::Path(p)) if !p.is_dir() => {}
        Some(t) => start_scan(core.clone(), t),
        None => {}
    }
    let icon_bytes = include_bytes!("../assets/icon-256.png");
    let icon = image::load_from_memory(icon_bytes).ok().map(|i| {
        let rgba = i.to_rgba8();
        let (w, h) = rgba.dimensions();
        egui::IconData {
            rgba: rgba.into_raw(),
            width: w,
            height: h,
        }
    });
    let mut viewport = egui::ViewportBuilder::default()
        .with_inner_size([1580.0, 980.0])
        .with_min_inner_size([900.0, 620.0])
        .with_maximized(true)
        .with_title("Diskhoji")
        .with_app_id("diskhoji");
    if let Some(ic) = icon {
        viewport = viewport.with_icon(Arc::new(ic));
    }
    let options = eframe::NativeOptions {
        viewport,
        ..Default::default()
    };
    let _ = eframe::run_native(
        "Diskhoji",
        options,
        Box::new(move |cc| Ok(Box::new(Native::new(cc, core, takeover)))),
    );
}

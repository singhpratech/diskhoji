...
/// Build a command for a desktop action (browser, file manager, terminal).
/// When Diskhoji runs elevated on Linux it is root inside a scrubbed pkexec
/// environment: no session bus, and browsers refuse to start as root. So the
/// action is run as the desktop user who asked for elevation, with that
/// user's session variables restored. Purely local — nothing leaves the machine.
fn desktop_cmd(prog: &str) -> Command {
    #[cfg(target_os = "linux")]
    {
        if elevate::is_elevated() {
            if let Some(uid) = std::env::var("DISKHOJI_USER_UID")
                .ok()
                .and_then(|v| v.parse::<u32>().ok())
                .filter(|&u| u != 0)
            {
                // runuser wants a name, not "#uid"
                let name = unsafe {
                ...
                c.arg("-u").arg(name).arg("--").arg("env");
                for k in ["DBUS_SESSION_BUS_ADDRESS", "XDG_RUNTIME_DIR", "HOME"] {
                    if let Ok(v) = std::env::var(format!("DISKHOJI_USER_{k}")) {
                        c.arg(format!("{k}={v}"));
                    }
                }
                c.arg(prog);
...
fn reveal_in_file_manager(path: &str) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if !ok {
            ...
            let _ = desktop_cmd("xdg-open").arg(parent).spawn();
        }
    }
}
...
fn open_with_default(path: &str) {
    ...
    #[cfg(target_os = "linux")]
    let _ = desktop_cmd("xdg-open").arg(path).spawn();
}
...
fn open_terminal(path: &str) {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(term) = std::env::var("TERMINAL") {
            if desktop_cmd(&term).current_dir(&dir).spawn().is_ok() {
                return;
            }
        }
        for (cmd, args) in [
            ...
            if desktop_cmd(cmd).args(args).current_dir(&dir).spawn().is_ok() {
                return;
            }
        }
    }
...
fn open_link(url: &str) {
    ...
    #[cfg(target_os = "linux")]
    {
        if desktop_cmd("xdg-open").arg(url).spawn().is_ok() {
            return;
        }
        for b in ["sensible-browser", "x-www-browser", "firefox", "google-chrome", "chromium"] {
            if desktop_cmd(b).arg(url).spawn().is_ok() {
                return;
            }
        }
    }
...
fn open_browser(url: &str) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let _ = desktop_cmd("xdg-open").arg(url).spawn();
    }
}

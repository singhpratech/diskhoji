...
- **One standalone binary, truly native.** The executable *is* the app: scanner, treemap engine, and a real native window (egui) in a single ~8 MB file. No installer, no Electron's 200 MB, no webview library. Copy it to any machine and run it.
...
- **Numbers that match your drive.** Diskhoji counts what files actually occupy on disk: hardlinked files once (Windows' WinSxS/System32 share thousands of DLLs), sparse and NTFS-compressed files at their real footprint, and OneDrive-style cloud placeholders at their true local size — so the total lines up with what Explorer or `df` reports as used, instead of overshooting it.
...
- **Trash first, permanent second.** Deleting sends items to the system trash / recycle bin by default (restorable); *Delete permanently* is a separate, explicit button. Ctrl-click selects several items and deletes them together; the Tools menu can empty the trash.
- **Every column WinDirStat has.** Explorer shows name · items · last change · % · size, each header sortable. A properties dialog, *open terminal here*, *rescan this folder* (in place, no full rescan), *locate in explorer*.
- **Find, filter, scan everything.** Name search with `*`/`?` wildcards; skip hidden files; exclude patterns (`node_modules, *.iso`) — also as `--skip-hidden` / `--exclude`; *All volumes* scans every local filesystem into one map (`--all`).
- **Map options.** Colour by file type, by top-level folder, or by age (last modified); cushion shading, outlines and labels toggles; *free space* and *unknown* blocks on a volume root so the map equals the whole drive (WinDirStat-style).
- **Export & keyboard.** CSV export of the whole scan or any folder (`path,type,bytes,files,modified`); ↑↓←→ Enter Backspace Del Ctrl+C/O/F/E/T/A, F5.
...
- **Sees everything — with your say-so.** Launched as a normal user, Diskhoji offers (WinDirStat-style) to restart itself with administrator rights, so system areas and other users' folders get measured instead of showing up unreadable. The prompt is the standard local one — polkit/`pkexec` on Linux, UAC on Windows, the administrator dialog on macOS — and elevation never touches the network. Decline for now, or tick *Don't ask again* (stored in `~/.config/diskhoji-elevate`); a link on the landing screen brings the offer back any time. In `--web` mode, run `sudo diskhoji --web` (or an administrator shell) for the same full coverage.
- **A game on board.** *The Seeker's Voyage* — a Chrome-dino-style side-scroller — ships inside the app. See [the mini-game](#the-seekers-voyage-the-built-in-mini-game) below.
...
cargo build --release
./target/release/diskhoji [PATH | --all] [--web] [--port N] [--no-open] [--skip-hidden] [--exclude PATTERN]
```
...
- **Play game** — The Seeker's Voyage mini-game, right in the app (native and `--web`): Space/↑ jump · release early for a short hop · ↓ dive · `Esc` back to work.

## The Seeker's Voyage (the built-in mini-game)

A Chrome-dino-style side-scroller set on Diskhoji's night sea — sail east under the pasha moon, leap the corrupt reefs, gather the amber bytes. It ships **inside the app**: click **Play game** on the landing screen (while you decide what to scan) or in the dashboard toolbar. The identical game runs in `--web` mode and on [diskhoji.org](https://diskhoji.org/#voyage).

| Control | Action |
|---|---|
| `Space` / `↑` / click or tap | Jump (also starts the run) |
| Release the key early | **Short hop** — jump height is variable, exactly like the Chrome dino |
| `↓` in mid-air | **Dive** — kills the jump and drops back to the water at 3× |
| `Esc` | Pause ("At anchor") and return to the app — resume any time |

**How the sea behaves:**

- The current starts gentle and **accelerates steadily for about two minutes** to full tide — Chrome-dino-style constant acceleration, no sudden difficulty cliffs.
- **Reef gaps stretch as the water speeds up**, so a faster sea never becomes unfair; clearability of the worst reef at every speed is verified with a frame-exact simulation before each release.
- **Fathoms** log your distance; each **amber byte** is worth 8. Tall reefs need a full-arc jump — short hops clear only the small ones. That's the skill.
- Your **best voyage is saved locally** (`~/.config/diskhoji-voyage-best` on Linux/macOS, the equivalent config dir on Windows; `localStorage` in the browser) and never sent anywhere — the game, like the app, is fully offline.
...
- Sizes are real on-disk usage (allocated bytes), shown in binary units (KiB/MiB/GiB) — so the scanned total agrees with your drive's used/free numbers.
...
`/api/status` · `/api/roots` · `/api/pick` · `/api/scan` · `/api/summary` · `/api/node/{id}` · `/api/treemap?id&w&h` · `/api/delete` · `/api/reveal` · `/api/open`

...
fn route(app: &Arc<App>, method: &Method, path: &str, url: &str, body: &str) -> Reply {
    match (method, path) {
        (Method::Get, "/") => Ok(Response::from_data(UI.as_bytes().to_vec())
            .with_header(
        ...
        (Method::Post, "/api/delete") => {
            ...
            let s = store.as_mut().ok_or((404, "no scan yet".to_string()))?;
            if r.id as usize >= s.nodes.len() || !s.nodes[r.id as usize].alive {
                return Err((404, "no such node".to_string()));
            }
            let target = scan::path_of(s, r.id);
            let is_dir = s.nodes[r.id as usize].is_dir;
            ...
            let (freed, files) = scan::remove_subtree(s, r.id);

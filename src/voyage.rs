...
// Chrome dino constants × K (dino world is 600px wide; ours is 1040).
const K: f32 = W / 600.0;
const BASE_SPEED: f32 = 6.0 * K; // dino SPEED
const MAX_SPEED: f32 = 13.0 * K; // dino MAX_SPEED
const ACCEL: f32 = 0.001 * K; // dino ACCELERATION, per frame
const GRAVITY: f32 = 0.6 * K; // dino GRAVITY, per frame²
const JUMP_V: f32 = 10.0 * K; // dino INITIAL_JUMP_VELOCITY (magnitude)
const DROP_V: f32 = 5.0 * K; // dino DROP_VELOCITY (jump-cut clamp)
const MIN_JUMP: f32 = 30.0 * K; // dino MIN_JUMP_HEIGHT before a cut is allowed
const SPEED_DROP_V: f32 = 1.0 * K; // dino sets jumpVelocity=1 on speed-drop
const REEF_MIN_GAP: f32 = 120.0 * K * 0.6; // dino minGap 120 × gapCoefficient 0.6
const SCORE_COEFF: f32 = 0.025 / K; // dino distance meter coefficient
...
impl Voyage {
    ...
    fn jump(&mut self) {
        ...
        if !self.jumping {
            self.jumping = true;
            self.vy = -(JUMP_V + self.speed / 10.0);
            self.reached_min = false;
            self.speed_drop = false;
        }
    }
    ...
    fn step(&mut self, dt_secs: f32) -> bool {
        ...
        self.t += dt;
        // dino: distanceRan += speed per frame; score = distance × 0.025.
        self.dist += self.speed * dt;
        // dino: constant acceleration every frame up to MAX_SPEED.
        ...
        if self.next_reef <= 0.0 {
            ...
            // dino getGap: minGap = width×speed + typeMinGap×gapCoefficient,
            // maxGap = 1.5×minGap — gaps stretch as the world speeds up.
            let min_gap = w * self.speed + REEF_MIN_GAP;

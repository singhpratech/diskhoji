...
impl Voyage {
    ...
    fn step(&mut self, dt_secs: f32) -> bool {
        ...
        for o in &mut self.orbs {
            // the orb bobs; collision must use the BOBBED position (same formula
            // the painter draws with), else pickups never register
            o.dy = o.y + 3.0 * (self.t * 0.07 + o.x * 0.05).sin();
    ...
    fn paint(&self, ui: &mut egui::Ui, rect: Rect) {
        ...
        let mc = p(mx, my);
        // layered halo approximates the web canvas shadowBlur glow
        for (w, a) in [(30.0, 12u8), (20.0, 22), (13.0, 36), (9.0, 50)] {
            painter.circle_stroke(
                mc,
                mr * s,
                Stroke::new(w * s, Color32::from_rgba_unmultiplied(0xE5, 0xB9, 0x6B, a)),
            );
        }
        ...
        // 4px strips so the crest doesn't stair-step like 8px ones did
        let mut x = 0.0;
        while x <= W {
            let y = self.wave_y(x + 2.0);
            painter.rect_filled(Rect::from_min_max(p(x, y), p(x + 4.0, H)), 0.0, SEA_FILL);
            x += 4.0;
        }
        // wave crest
        ...
        for ob in &self.orbs {
            ...
            let c = p(ob.x, y);
            // layered glow ≈ the web canvas shadowBlur 10
            for (r, a) in [(16.0, 20u8), (12.0, 38), (9.0, 60)] {
                painter.circle_filled(c, r * s, Color32::from_rgba_unmultiplied(0xE5, 0xB9, 0x6B, a));
            }
    ...
    fn paint_boat(&self, painter: &egui::Painter, p: &dyn Fn(f32, f32) -> Pos2, s: f32) {
        ...
        // lantern glow + body — layered falloff ≈ the web radial gradient
        for (r, a) in [(14.0, 28u8), (10.0, 55), (7.0, 95), (4.5, 150)] {
            painter.circle_filled(
                q(40.0, -16.0),
                r * s,
                Color32::from_rgba_unmultiplied(0xE5, 0xB9, 0x6B, a),
            );
        }
        painter.rect_filled(
...
mod tests {
    ...
    /// Regression test for the v0.2.7 bug where `Orb::dy` was never written
    /// (the painter computed the bob but couldn't store it), so the pickup
    /// check compared against 0 and amber bytes could never be collected.
    #[test]
    fn orbs_are_collectable() {
        let mut v = Voyage::new(0);
        v.start();
        let mut frames = 0u32;
        while frames < 120_000 && v.bytes == 0 {
            if !v.running {
                v.start();
            }
            // pilot: leap reefs to survive (timing from the clearability sim),
            // and leap for orbs when the water ahead is clear
            let reef_near = v
                .reefs
                .iter()
                .any(|r| r.x > BOATX && r.x < 150.0 + 18.0 * v.speed);
            let reef_far = v.reefs.iter().all(|r| r.x < BOATX || r.x > 650.0);
            let orb_near = v
                .orbs
                .iter()
                .any(|o| !o.got && o.x > BOATX + 40.0 && o.x < BOATX + 160.0);
            if !v.jumping && (reef_near || (orb_near && reef_far)) {
                v.jump();
            }
            v.step(1.0 / 60.0);
            frames += 1;
        }
        assert!(v.bytes > 0, "no amber bytes collected in {frames} frames");
    }
}
